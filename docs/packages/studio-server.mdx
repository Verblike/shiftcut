---
title: "@shiftcut/studio-server"
description: "The studio preview/editor backend — a mountable Hono API, extracted from core."
---

The studio-server package is the HTTP backend that powers studio preview and editing — project routes, file serving, preview bundling, thumbnails, and source-mutation helpers. It was extracted from `@shiftcut/core/studio-api` into a dedicated package so an embedder can mount the studio backend **without** depending on core's full surface, and so core no longer ships a web server it doesn't need at render time.

```bash
npm install @shiftcut/studio-server
```

## When to Use

<Tip>
  **Most users do not need this package directly.** The [CLI](/packages/cli) (`npx shiftcut preview`) and [studio](/packages/studio) wire it up for you. Reach for it when you're **embedding** the studio backend into your own server.
</Tip>

**Use `@shiftcut/studio-server` when you need to:**
...
<Info>
  `@shiftcut/core/studio-api` still resolves (via a back-compat re-export stub), so existing imports keep working. New code should import from `@shiftcut/studio-server` directly.
</Info>
...
| Import | Description |
|--------|-------------|
| `@shiftcut/studio-server` | `createStudioApi`, helpers, types |
| `@shiftcut/studio-server/source-mutation` | Source mutation utilities |
| `@shiftcut/studio-server/screenshot-clip` | Element screenshot-clip geometry |
| `@shiftcut/studio-server/manual-edits-render-script` | Manual-edits render body script |
| `@shiftcut/studio-server/studio-motion-render-script` | Studio motion render body script |
| `@shiftcut/studio-server/draft-markers` | Draft gesture-marker attributes |
| `@shiftcut/studio-server/finite-mutation` | Finite-mutation safety checks |
...
```typescript
import { createStudioApi } from '@shiftcut/studio-server';
import type { StudioApiAdapter, ResolvedProject } from '@shiftcut/studio-server';

const adapter: StudioApiAdapter = {
  listProjects: () => projects,
  resolveProject: (id) => projectsById.get(id) ?? null,
  bundle: async (projectDir) => bundleToSingleHtml(projectDir),
  lint: (html, opts) => lintShiftCutHtml(html, opts),
  runtimeUrl: '/shiftcut-runtime.js',
...
<CardGroup cols={2}>
  ...
  <Card title="CLI" icon="terminal" href="/packages/cli">
    `npx shiftcut preview` wires this server up for you.
  </Card>
</CardGroup>
