...
export interface EngineConfig {
   ...
   * `<tmpdir>/shiftcut-extract-cache-<uid>`.
   *
   ...
   *
   * Set `SHIFTCUT_EXTRACT_CACHE_DIR` to a path to override the default, or
   ...
   *
   * Env fallback: `SHIFTCUT_EXTRACT_CACHE_DIR`.
   */
   ...
   *
   * Env fallback: `SHIFTCUT_EXTRACT_CACHE_MAX_MB` (megabytes).
   */
...
/** Default configuration — sensible for Shiftcut compositions. */
 ...
 * the wider {@link resolveConfig} pipeline so `shiftcut doctor` (and any
 ...
 * `SHIFTCUT_EXTRACT_CACHE_DIR`. Exported so downstream tests can reproduce
 * the exact path without duplicating the uid-suffix idiom.
 */
export function defaultExtractCacheDir(): string {
  return join(tmpdir(), `shiftcut-extract-cache-${process.getuid?.() ?? "u"}`);
}
...
export function resolveConfig(overrides?: Partial<EngineConfig>): EngineConfig {
  ...
  const fromEnv: Partial<EngineConfig> = {
    ...
    extractCacheMaxBytes:
      envNum("SHIFTCUT_EXTRACT_CACHE_MAX_MB", DEFAULT_CONFIG.extractCacheMaxBytes / 1024 ** 2) *
      1024 ** 2,
  };
  ...
  if (
    ...
    console.error(
      "[shiftcut] Windows compound-workaround auto-detected — disabling streaming-encode " +
