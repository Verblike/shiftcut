...
export interface RenderConfig {
   ...
   * without a decimal round-trip — see `fpsToFfmpegArg` in @shiftcut/core.
   *
...
async function executeRenderPipeline(input: {
  ...
  try {
    ...
    recordTransientRetryObservability();
    // Surface ShiftCut' PRODUCER_PUPPETEER_PROTOCOL_TIMEOUT_MS env +
    ...
    // setting" text doesn't name the ShiftCut knob and doesn't state the
    ...
    // Surface ShiftCut' PRODUCER_PAGE_NAVIGATION_TIMEOUT_MS env +
    // --browser-timeout CLI + SHIFTCUT_BROWSER_PATH escape hatch in
    ...
    // SHIFTCUT_BROWSER_PATH surfacing at the runtime-navigation layer
