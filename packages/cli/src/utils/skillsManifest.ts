// Skills freshness: give the ShiftCut skill bundle a content fingerprint so
...
export interface SkillsManifest {
  /** Source repo, e.g. "Vadagon/shiftcut". */
  source: string;
...
const DEFAULT_REPO_SLUG = "Vadagon/shiftcut";
...
//
//   core     — the `/shiftcut` entry router plus the shared domain skills
//              (`shiftcut-*`, `media-use`) that every creation workflow
//              references structurally (sibling `../shiftcut-animation/…`
...
/**
 * Find the first skill root that actually contains ShiftCut skills. A
 ...
 * scope, and ShiftCut now installs globally. Checking global-first means
...
export function diffSkills(
  ...
  return {
    ...
    // triggered (`shiftcut skills update <name>`). Counting it here is what
...
//
// `skills add` / `init` / `shiftcut skills update` only ever add or refresh —
...
/**
 * The skill names the upstream lock attributes to ShiftCut, for a scope.
 ...
 */
export function shiftcutSkillNames(opts: {
  scope: "project" | "global";
  cwd?: string;
  home?: string;
}): string[] {
