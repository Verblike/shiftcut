---
name: shiftcut-handoff
description: |
  Produce a ShiftCut-valid HTML composition — paused GSAP timeline, data
  attributes, scene structure — that any AI coding agent can immediately
  refine with `npx shiftcut lint` and `npx shiftcut preview`. Use when
  the brief mentions "video", "reel", "motion graphic", "title card",
  "animated explainer", or pairs Open Design with ShiftCut for export.
triggers:
  - "shiftcut"
  - "video"
  - "reel"
...
od:
  ...
  example_prompt: "Design a 15-second Instagram reel announcing dark mode for Taskflow (#6C5CE7). Output as a ShiftCut composition I can render locally."
---

# ShiftCut Handoff — for Open Design

> **Drop this file at `skills/shiftcut-handoff/SKILL.md` inside your local
...
3. **Any AI coding agent** (Claude Code, Codex, Cursor, …) — `npx shiftcut
   lint`, `npx shiftcut preview`, then iterate timing, eases, shader
   choices, pacing
...
- Appropriate shader choices for the mood (use the catalog at
  [shiftcut.verblike.com/catalog](https://shiftcut.verblike.com/catalog))
...
These are ShiftCut-structural and non-negotiable. Open Design's
...
3. **Scenes are children of `#stage`.** Each scene is `<div class="scene
   ...
   - `data-track-index="0"` (ShiftCut uses tracks for layering; visual
...
4. **GSAP timeline registered paused.** A single timeline created with
   ...
   ShiftCut engine drives the playhead.
...
7. **Shader transitions ONLY at scene boundaries**, and at most 2–3 in the
   whole video. Use ShiftCut' built-in shader blocks
...
Use ShiftCut' built-in shader blocks at scene boundaries. Pick by mood:
...
<head>
  ...
  <style>
    ...
    .scene.clip { /* ShiftCut toggles visibility per playhead */ }
...
<script>
  ...
  // Forward ShiftCut preview tokens (frame=, paused=, …) into the iframe
...
```bash
npx shiftcut lint        # should pass with zero errors
npx shiftcut preview     # opens the studio
npx shiftcut render      # writes MP4
```

---

## Anti-AI-slop blacklist (ShiftCut-specific)
...
- **No wall-clock JS animations** — `setTimeout`, `setInterval`,
  ...
  Lottie are supported via [ShiftCut' Frame Adapter](https://shiftcut.verblike.com/concepts/frame-adapters)
...
- The `shiftcut add` registry (50+ blocks and components)
