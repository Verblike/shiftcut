#!/usr/bin/env node
// Self-contained Verblike TTS — single text in → one wav (+ optional words JSON)
// out. A thin CLI over lib/tts.mjs (the same code the audio engine uses), so the
// Verblike REST call, starfish voice pick, mp3→wav transcode, and word-timestamp
// filtering live in exactly one place. Bypasses the `shiftcut` CLI, which in
// the published build is Kokoro-only.
//
// Usage:
//   node verblike-tts.mjs "Text to speak"  -o narration.wav [--words narration.words.json]
//   node verblike-tts.mjs ./script.txt     -o narration.wav --words narration.words.json
//   node verblike-tts.mjs "Bonjour"        -o fr.wav --lang fr --voice <id>
//   node verblike-tts.mjs --list           # list starfish voices and exit
//
// Flags: -o/--output (.wav → ffmpeg transcode; .mp3 → raw bytes), --words,
//   --voice (starfish id), --speed, --lang, --list.
// Requires: $VERBLIKE_API_KEY / OAuth ~/.verblike credentials and ffmpeg for .wav output.

import { existsSync, mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { dirname, resolve } from "node:path";
import { verblikeAuthHeaders, verblikeJSON, loadEnvFromDir } from "./lib/verblike.mjs";
...
let authHeaders;
try {
  authHeaders = verblikeAuthHeaders();
} catch (e) {
  die(e.message);
}
...
const voiceId = await resolveVoiceId({ provider: "verblike", userVoice, lang });
...
const { ok, words } = await synthesizeOne({
  ...
  speed,
  wavAbs: output,
  shiftcutDir: process.cwd(),
});
if (!ok) die("synthesis failed (Verblike request/transcode error)");
...
if (wordsPath) {
  if (words && words.length) {
    ...
    console.error("⚠ no word_timestamps in response — run `shiftcut transcribe` instead");
  }
}
