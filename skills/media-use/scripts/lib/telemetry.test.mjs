...
function withoutTelemetryOptOut() {
  for (const k of ["DO_NOT_TRACK", "SHIFTCUT_NO_TELEMETRY", "CI", "NODE_ENV"])
...
test("optedOut respects DO_NOT_TRACK / SHIFTCUT_NO_TELEMETRY / CI", () => {
  ...
  try {
    ...
    assert.equal(optedOut(), true, "SHIFTCUT_NO_TELEMETRY opts out");
...
test("track is a no-op (no network, resolves) when opted out", async () => {
  ...
  try {
    ...
    assert.equal(existsSync(join(home, ".shiftcut/config.json")), false);
...
test("anonymous id uses the shared shiftcut config", () => {
  const savedEnv = { ...process.env };
  const { root, home } = sandbox();
  try {
    withoutTelemetryOptOut();
    mkdirSync(join(home, ".shiftcut"), { recursive: true });
    writeFileSync(
...
test("anonymous id seeds missing config once and reuses it", () => {
  ...
  try {
    ...
    const configPath = join(home, ".shiftcut/config.json");
...
test("anonymous id adopts a legacy ~/.media/anon-id on upgrade (persona continuity)", () => {
  ...
  try {
    ...
    // no ~/.shiftcut/config.json yet — the old media-use-only id must carry over
    ...
    assert.equal(
      JSON.parse(readFileSync(join(home, ".shiftcut/config.json"), "utf8")).anonymousId,
      "legacy-media-id",
    );
  } finally {
    restoreEnv(savedEnv);
...
test("track identifies a signed-in Verblike account once and still sends events", async () => {
  const savedEnv = { ...process.env };
  const originalFetch = globalThis.fetch;
  const { root, home } = sandbox();
  const calls = [];
...
test("first run notice prints to stderr once and never stdout", async () => {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(readFileSync(join(home, ".shiftcut/config.json"), "utf8")).telemetryNoticeShown,
      true,
    );
  } finally {
    globalThis.fetch = originalFetch;
...
test("read-only telemetry state degrades without throwing", async () => {
  ...
  try {
    withoutTelemetryOptOut();
    writeFileSync(join(home, ".shiftcut"), "not a directory");
    writeFileSync(join(home, ".media"), "not a directory");
    await track("media_use_resolve", { type: "bgm" });
  } finally {
    globalThis.fetch = originalFetch;
    try {
      chmodSync(join(home, ".shiftcut"), 0o600);
    } catch {
