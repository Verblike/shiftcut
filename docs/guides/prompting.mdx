...
```bash
npx skills add Vadagon/shiftcut
```

The installer shows a picker. Select the **core skills** below — every project needs them. In Claude Code, restart the session after installing; the skills register as **slash commands**. Start at `/shiftcut`: it orients you to the whole surface and routes "make me a video" requests to the right workflow.

**Core skills — install all of these**

| Slash command           | What it loads                                                              |
| ----------------------- | -------------------------------------------------------------------------- |
| `/shiftcut`          | **Read first.** The entry skill — capability map + video router; sends "make me a video" intent to the right workflow |
| `/shiftcut-core`     | Composition contract — HTML structure, `data-*` attributes, clips, tracks  |
| `/shiftcut-animation`| All animation — motion rules, scene blueprints, transitions, and the runtime adapters (GSAP, Lottie, Three.js, Anime.js, CSS, WAAPI, TypeGPU) |
| `/shiftcut-creative` | Creative direction — design spec, palettes, typography, narration, beats   |
| `/shiftcut-cli`      | Dev-loop CLI — `init`, `lint`, `validate`, `inspect`, `preview`, `render`, `doctor` |
| `/media-use`    | Asset preprocessing — `tts`, `transcribe`, `remove-background`             |
| `/shiftcut-registry` | Block and component installation via `shiftcut add`                     |
| `/general-video`        | The general authoring workflow — fallback for any video that doesn't match a specific workflow below |

**Optional workflows — add the ones that match your inputs** (`/shiftcut` routes to whichever you've installed)
...
| `/remotion-to-shiftcut` | Port an existing Remotion (React) composition to ShiftCut HTML         |

<Tip>
  To skip the picker and install everything (core + every workflow) in one shot, run `npx skills add Vadagon/shiftcut --all`. And start Shiftcut prompts with `/shiftcut` (or invoke the skill another way for non-Claude agents) — it loads the routing + composition context explicitly so the agent picks the right workflow and gets the rules right the first time.
</Tip>

## Claude Design

Claude Design uses a different setup. Download [`claude-design-shiftcut.md`](https://github.com/Vadagon/shiftcut/blob/main/docs/guides/claude-design-shiftcut.md) from GitHub (click the ↓ button), then **attach it to your chat** (don't paste the URL — file attachments produce better output):
...
Claude Design produces a valid first draft (brand identity, scene content, animations, transitions). Download the ZIP and refine in any AI coding agent with `npx shiftcut preview` running. See the [Claude Design guide](/guides/claude-design) for the full workflow.

## The two prompt shapes

Most successful Shiftcut prompts fall into one of two shapes.
...
> Using `/shiftcut`, create a 10-second product intro with a fade-in title over a dark background and subtle background music.

> Make a 9:16 TikTok-style hook video about [topic] using `/shiftcut`, with bouncy captions synced to a TTS narration.
...
You give the agent something to work with — a URL, a doc, a CSV, a transcript — and ask it to synthesize that into a video. This is where Shiftcut shines because the agent does the research/summarization step *and* the production step in one flow.

> Take a look at this GitHub repo https://github.com/Vadagon/shiftcut and explain its uses and architecture to me using `/shiftcut`.

> Summarize the attached PDF into a 45-second pitch video using `/shiftcut`.

> Read this changelog and turn the top three changes into a 30-second release announcement video using `/shiftcut`.

> Turn this CSV into an animated bar chart race using `/shiftcut`.
...
Shiftcut is a conversation. After the first render, talk to the agent the way you'd talk to a video editor — don't re-prompt from scratch:
...
ShiftCut supports three TTS providers: **Verblike** (Starfish voices, requires sign-in via `npx shiftcut auth`), **ElevenLabs** (requires API key), and **Kokoro** (free, runs locally, no API key needed). The agent asks which provider to use — or picks automatically in autonomous mode. Describe the content and the agent picks a voice, or request one directly:
...
"Use Verblike TTS for this narration"
```
...
- **Don't ask for React / Vue components.** Shiftcut compositions are plain HTML with `data-*` attributes and a GSAP timeline. Asking for "a React component for the intro" forces the agent to translate later.
...
- **Don't skip the slash command.** Without `/shiftcut`, the agent may guess at HTML video conventions instead of using the framework's actual rules (`class="clip"` on timed elements, `window.__timelines` registration, etc.).
- **Don't paste long error logs into the prompt without context.** Run `npx shiftcut check` first — lint catches structural issues, validate catches runtime errors (JS exceptions, missing assets, contrast problems).
...
1. `npx shiftcut init my-video` — scaffold a project (skills install automatically)
2. Open the project in Claude Code (or Cursor / Codex)
3. Prompt with `/shiftcut` and one of the shapes above
4. `npx shiftcut preview` — watch in the browser as the agent edits
5. Iterate with small targeted prompts
6. `npx shiftcut render --output final.mp4` when you're happy
