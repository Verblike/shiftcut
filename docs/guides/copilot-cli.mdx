...
description: "Set up ShiftCut with GitHub Copilot CLI — install skills, invoke them with slash commands, and render video from the terminal."
---

GitHub Copilot CLI brings AI-powered coding assistance to your terminal. ShiftCut skills teach it how to write correct compositions, GSAP timelines, and framework-specific patterns — so you get valid video HTML without reading the docs yourself.
...
- **Node.js 22+** and **FFmpeg** for the ShiftCut CLI
...
<Tip>
  ...
  ```bash
  npx skills add Verblike/shiftcut --agent github-copilot --global
  ```
...
```
/shiftcut Create a 10-second product intro with a fade-in title and dark background.
```

```
/shiftcut-animation Add a scale-pop animation to the title element.
```

```
/shiftcut-cli How do I render at 60fps with Docker?
```
...
<Steps>
  ...
  <Step title="Start the preview server">
    ...
    Opens the ShiftCut Studio in your browser. Edits reload automatically.
  </Step>
  ...
  <Step title="Prompt with the skill">
    ```
    /shiftcut Create a 15-second dark-themed product intro with hype-style
    captions and a flash transition to the CTA.
    ```

    The agent writes valid ShiftCut HTML — `data-*` attributes, `class="clip"` on timed elements, paused GSAP timelines registered on `window.__timelines`. The preview updates as files are saved.
  </Step>
  ...
  <Step title="Render">
    ...
    ```
    /shiftcut-cli Render this composition to output.mp4 at high quality.
    ```
  </Step>
</Steps>
...
```
/shiftcut Scaffold a new project called "launch-video", create a 30-second
product launch video with 5 scenes, install the flash-through-white transition
block, and render to mp4.
```

In agent mode, Copilot runs terminal commands (like `npx shiftcut init` and `npx shiftcut render`) on your behalf. Review the commands before approving them — especially if the skill pre-approves `shell` in its `allowed-tools`.

<Warning>
  Only pre-approve `shell` or `bash` in skill `allowed-tools` for skills you trust. The ShiftCut skills do not pre-approve shell access — the agent will ask for confirmation before running terminal commands.
</Warning>

## MCP alternative

Copilot CLI also supports MCP servers for cloud-based authoring without the local CLI. Use the `--additional-mcp-config` flag to add the ShiftCut MCP server to your session:

```bash
copilot --additional-mcp-config '{"mcpServers":{"shiftcut":{"url":"https://shiftcut.verblike.com/mcp/shiftcut"}}}'
```
...
- **Always start with `/shiftcut`.** It's the entry skill — it orients you to the whole surface and routes your request to the right workflow, pulling in the domain skills (`/shiftcut-core` for composition rules and data attributes, `/shiftcut-animation` for GSAP and other runtime patterns) that generic web knowledge doesn't cover.
- **Use `/skills` to check skill status.** If output looks wrong, open the skills picker to verify the ShiftCut skills are enabled.
- **Run `npx shiftcut lint` before rendering.** The linter catches structural issues the agent might miss on complex multi-scene edits.
...
- **Install registry blocks for transitions and effects.** `npx shiftcut add shimmer-sweep` installs pre-built blocks, then prompt the agent to wire them in.
