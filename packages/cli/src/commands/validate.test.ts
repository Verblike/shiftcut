...
// mock it so these unit tests never resolve @shiftcut/producer's built dist.
...
describe("validate command deprecation (U5)", () => {
  ...
  it("prints a one-line deprecation notice to stderr and never to stdout", async () => {
    ...
    expect(stderrText).toContain("shiftcut validate");
    expect(stderrText).toContain("shiftcut check");
