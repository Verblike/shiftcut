...
function warnColorGradingLutNotInlined(lutSrc: string): void {
  ...
  console.warn(
    `[ShiftCut] Could not inline color grading LUT "${trimmed}". The rendered bundle may not be self-contained.`,
  );
}
...
function injectTextRenderingRule(document: Document): void {
  ...
  if (!head) return;
  if (document.querySelector("style[data-shiftcut-text-rendering]")) return;

  const styleEl = document.createElement("style");
  styleEl.setAttribute("data-shiftcut-text-rendering", "true");
...
export interface BundleOptions {
  ...
  /**
   * How to handle the ShiftCut runtime <script> tag. Default: `"inline"`.
   *
 ...
 * - Injects the ShiftCut runtime script
      ...
      opts.compScriptChunks.push(
        opts.compId
          ? wrapScopedCompositionScript(
              ...
              opts.compId,
              "[ShiftCut] composition script error:",
              opts.runtimeScope,
...
export async function bundleToSingleHtml(
  ...
  const subCompResult = inlineSubCompositions(document, subCompositionHosts, {
    ...
    scriptErrorLabel: "[ShiftCut] composition script error:",
