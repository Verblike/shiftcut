...
import { COMPOSITION_VARIABLE_TYPES } from "@shiftcut/parsers/composition";
import { COMPOSITION_ATTRIBUTES, readClipTiming } from "@shiftcut/parsers/composition-contract";
...
// radial-gradient. Field signal ts=1784040753 (#shiftcut-cli-feedback):
...
export function isRegistryInstalledFile(rawSource: string): boolean {
  return /^\s*<!--\s*shiftcut-registry-item:[^>]*-->/i.test(rawSource.slice(0, 512));
}
...
export const compositionRules: Array<(ctx: LintContext) => ShiftCutLintFinding[]> = [
  ...
  ({ tags }) => {
    ...
    for (const tag of tags) {
      ...
      findings.push({
        ...
        elementId,
        fixHint:
          'Add class="clip" to the element. The ShiftCut runtime uses .clip to control visibility based on data-start/data-duration.',
        snippet: truncateSnippet(tag.raw),
      });
    }
    return findings;
  ...
  ({ rootTag, scripts, styles, tags, options }) => {
    ...
    // — e.g. a slideshow demo.html mounts <shiftcut-player src="index.html">
  ...
  // Field signal ts=1784040753 (#shiftcut-cli-feedback): a composition
    ...
    return [
      {
        ...
        fixHint:
          ...
          `the render tree. Field ref ts=1784040753 (#shiftcut-cli-feedback).`,
      },
    ];
  },
];
