// Opt-out usage tracking for media-use, sharing the shiftcut CLI/studio
// identity (packages/cli/src/telemetry): the same install id from
// ~/.shiftcut/config.json, plus a $identify to the Verblike account on sign-in,
...
// to ship), same opt-outs (DO_NOT_TRACK / SHIFTCUT_NO_TELEMETRY / CI / dev),
...
// CLI + studio share one install identity in ~/.shiftcut/config.json
...
function sharedConfigPath() {
  return join(homedir(), ".shiftcut", "config.json");
}
...
function writeSharedConfig(config) {
  const dir = join(homedir(), ".shiftcut");
  ...
  try {
    ...
    console.error(
      [
        ...
        "If you sign in to Verblike, usage links to your account email or username. Opt out with SHIFTCUT_NO_TELEMETRY=1 or DO_NOT_TRACK=1.",
      ].join("\n"),
    );
