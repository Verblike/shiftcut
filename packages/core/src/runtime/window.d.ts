...
declare global {
  interface Window {
     ...
     * passes `shiftcut render --variables '<json>'`. Read indirectly via
     * `window.__shiftcut.getVariables()` (or the named `getVariables`
     * export from `@shiftcut/core`), which merges these over the
