---
title: Open Design
description: "Create ShiftCut video drafts in Open Design — the open-source, BYOK Claude-Design alternative — then refine in any AI coding agent."
---
...
Open Design produces a **valid first draft** of a ShiftCut composition — palette, scene content, GSAP entrance tweens, mid-scene activity, and shader transitions. You then download the project and refine in any AI coding agent with linting and live preview.

## Get started

<Steps>
  <Step title="Download the instruction file">
    Open [`open-design-shiftcut.md`](https://github.com/Vadagon/shiftcut/blob/main/docs/guides/open-design-shiftcut.md) on GitHub and click the download button (↓) to save it.
  </Step>
  ...
  <Step title="Drop the file into a skill, or attach it to chat">
    **Recommended:** copy `open-design-shiftcut.md` to `skills/shiftcut-handoff/SKILL.md` inside the Open Design repo. The daemon auto-discovers it on the next request and exposes it as a skill in the picker. **Or:** start a new chat and attach the file directly — Open Design reads attachments natively.
  </Step>
  <Step title="Describe your video">
    Pick the `shiftcut-handoff` skill (or your active prototype skill), pick a design system or visual direction, and type the brief. Include screenshots, brand assets, or a palette if you have them.
  </Step>
  ...
  <Step title="Refine in any AI coding agent">
    ...
    ```bash
    cd .od/projects/<id>
    npx skills add Vadagon/shiftcut   # install skills (one-time)
    npx shiftcut lint                     # should pass with zero errors
    npx shiftcut preview                  # open the studio
    ```
  </Step>
</Steps>
...
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Open Design (open-source)              | Drop [`open-design-shiftcut.md`](https://github.com/Vadagon/shiftcut/blob/main/docs/guides/open-design-shiftcut.md) into `skills/shiftcut-handoff/SKILL.md`   |
| Claude Code                            | `npx skills add Vadagon/shiftcut`, then use `/shiftcut`                                                                                                             |
| Cursor / Codex / Gemini CLI            | `npx skills add Vadagon/shiftcut`                                                                                                                                       |
| Claude Design (closed-source)          | Attach [`claude-design-shiftcut.md`](https://github.com/Vadagon/shiftcut/blob/main/docs/guides/claude-design-shiftcut.md) to your chat                          |
...
<CardGroup cols={1}>
  <Card title="Feature announcement">
    ```text
    Use the shiftcut-handoff skill. I just shipped dark mode for my app.
  ...
  <Card title="Founder pitch (BYOK with your own CLI)">
    ```text
    Use the shiftcut-handoff skill on Codex CLI. 25-second LinkedIn video.
  ...
  <Card title="Brand-driven (use one of the 72 design systems)">
    ```text
    Use the shiftcut-handoff skill. Design system: Stripe. 10-second reel.
  ...
  <Card title="Sparse brief (let it ask)">
    ```text
    Use the shiftcut-handoff skill. 30-second launch video for Orbit.
    ```
...
- **Render still happens locally** — Open Design produces the HTML; `npx shiftcut render` and HDR encoding still need FFmpeg + Node 22+ on your machine.
- **In-pane preview is sandboxed iframe** — full browser playback is reliable; for frame-accurate scrubbing use `npx shiftcut preview` after handoff.
...
npx skills add Vadagon/shiftcut   # one-time setup
npx shiftcut lint                     # verify structure
npx shiftcut preview                  # open the studio
```
...
<CardGroup cols={2}>
  ...
  <Card title="Prompt Guide" icon="message" href="/guides/prompting">
    More prompt patterns for ShiftCut across Claude Code, Claude Design, Open Design, and other agents.
  </Card>
</CardGroup>
