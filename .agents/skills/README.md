...
Codex 0.144.3) when the `shiftcut` repo is opened as the working
...
3. Accept, and the agent produces `weekly-changelog-<range>.mp4` gated
   by `shiftcut check` (0 errors) + `seam-gate verify` (0 fail/warn).
...
- A headless Chrome for ShiftCut rendering (`shiftcut doctor`
