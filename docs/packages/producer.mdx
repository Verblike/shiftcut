...
```bash
npm install @shiftcut/producer
```

## When to Use

**Use `@shiftcut/producer` when you need to:**
...
- Render from the command line without writing code — use the [CLI](/packages/cli) (`npx shiftcut render`)
...
<Steps>
  ...
  <Step title="Inject the Shiftcut runtime">
...
```typescript
import { createRenderJob, executeRenderJob } from '@shiftcut/producer';
...
```typescript
import { createRenderJob } from '@shiftcut/producer';
...
```typescript
import type { ProgressCallback, RenderStatus } from '@shiftcut/producer';
...
```typescript
import { RenderCancelledError } from '@shiftcut/producer';
...
```typescript
import { startServer } from '@shiftcut/producer/server';
...
```typescript
import { createRenderHandlers, createProducerApp } from '@shiftcut/producer/server';
...
<Info>
  Docker mode requires Docker to be installed and running. Run `npx shiftcut doctor` to verify your environment. See [Deterministic Rendering](/concepts/determinism) for details on what makes Docker mode deterministic.
</Info>
...
When GPU encoding is enabled, Shiftcut detects the available FFmpeg hardware encoder automatically. To check your system's capabilities:
...
```typescript
import { resolveConfig } from '@shiftcut/producer';
...
| `prepareShiftCutLintBody()` / `runShiftCutLint()` | Linting utilities |
