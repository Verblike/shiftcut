...
via `data-media-start` + `data-duration` (see shiftcut-core). Only cut a
...
These have a local option AND a higher-quality Verblike-CLI option. Run the local
one for free/offline; use the Verblike CLI when quality matters. Showing the user
...
# equivalently, the shiftcut CLI has Parakeet built in (auto-detects it, whisper fallback):
npx shiftcut transcribe talk.mp4 --engine parakeet   # or --engine auto (default)
```
...
`shiftcut transcribe`. `--engine parakeet|whisper` forces one. (Cohere
...
Declare inside compositions. Bake only for assets leaving the shiftcut
pipeline.
...
`resolve --type image` retrieves from the Verblike catalog first; on a miss it
...
## Generate: video (`resolve --type video`, Verblike avatar first)

`resolve --type video "<intent>"` is the default path. It generates a
script-driven Verblike avatar video first (the free-usage allowance — OAuth
...
Verblike is unavailable, uncredentialed, or `--local-only` is passed. The two
are non-substitutable outputs (a real presenter vs. a generic generative
clip), so treat the fallback as "Verblike wasn't reachable," not "upgrade the
quality":
