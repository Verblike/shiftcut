---
title: ShiftCut MCP
description: "Author, preview, and render ShiftCut videos directly inside Claude.ai, ChatGPT, and Grok — no local install required."
---

The ShiftCut MCP is a hosted [Model Context Protocol](https://modelcontextprotocol.io) server that lets you create, edit, preview, and render ShiftCut video compositions from inside Claude.ai, ChatGPT, or Grok.

<Note>
  **In beta.** Features and pricing may change. Found a bug or have feedback? [File an issue on GitHub](https://github.com/Verblike/shiftcut/issues).
</Note>
...
The agent behind `compose` has 25+ ShiftCut-specific skills baked in — typography, color palettes, motion principles, GSAP effects, audio-reactive animation, and captions. You don't have to specify any of this directly; describe the video you want and the agent picks the right tools.
...
### 1. Get a Verblike account

The MCP requires a Verblike account for authentication and credits. Sign up at [verblike.com](https://shiftcut.verblike.com) if you don't have one.
...
<Tabs>
  <Tab title="Claude.ai">
    <Steps>
      ...
      <Step title="Find ShiftCut">
        Search the connector catalog for **ShiftCut** and click **Connect**.

        Don't see it in the catalog? Add it manually: **Add custom connector**, then paste:
        ```
        https://shiftcut.verblike.com/mcp/shiftcut
        ```
      </Step>
      <Step title="Sign in to Verblike">
        OAuth opens in a new window. Authorize the ShiftCut connector to access your Verblike account.
      </Step>
...
Authors a new composition or applies an edit to an existing one. The ShiftCut agent handles captions, blocks, layout, transitions, color, and timing internally based on your natural-language prompt.
...
### Reference your existing Verblike assets

If you've uploaded logos, fonts, or other assets to your Verblike account, the agent can use them. Just say *"use my logo"* or *"use my brand font."* The agent resolves the asset by name and recency.
...
```bash
npx @modelcontextprotocol/inspector npx -y mcp-remote https://shiftcut.verblike.com/mcp/shiftcut
```
...
<AccordionGroup>
  <Accordion title="OAuth keeps failing or loops">
    Verify you're using the production URL: `https://shiftcut.verblike.com/mcp/shiftcut`. The dev URL (`mcp.dev.verblike.com`) only accepts dev accounts.

    If OAuth completes but you see "not authorized" errors, your Verblike account may not have access to the MCP — contact support or check your tier.
  </Accordion>
  ...
  <Accordion title="Render is stuck or never completes">
    ...
    3. If the issue persists, file an issue at [github.com/Verblike/shiftcut/issues](https://github.com/Verblike/shiftcut/issues) including the `job_id` from `render_video`
  </Accordion>

  <Accordion title='"Composition not found" error'>
    The `composition_id` is owned by the Verblike space (account) that created it. If you're signed into a different space, you can't access compositions from another. Run `list_compositions` to see what's available from your current account.
  </Accordion>

  <Accordion title="Player widget shows blank or loads forever">
    Usually a transient connection issue between the widget and `mcp.verblike.com`. Refresh the chat or call `get_composition` again.
  ...
  <Accordion title="Out of credits">
    The MCP returns an error with an upgrade URL when your credits are exhausted. Visit [verblike.com/pricing](https://shiftcut.verblike.com/pricing) to upgrade your tier.
  </Accordion>
...
For bugs or feature requests, file an issue at [github.com/Verblike/shiftcut/issues](https://github.com/Verblike/shiftcut/issues). Include:
...
- **Cloud-only rendering.** All renders run on Verblike infrastructure. Use the [CLI](/quickstart) if you need local rendering.
- **Single-user.** Each composition is owned by one Verblike account. No team sharing in v1.
- **No binary uploads from chat.** You can reference assets you've already uploaded to Verblike via the web UI, but the MCP does not currently accept new file uploads through chat. Upload via [app.verblike.com](https://shiftcut.verblike.com) first, then reference the asset by name.
...
[ShiftCut itself is open source](https://github.com/Verblike/shiftcut) — the HTML composition format, CLI, renderer, and player. You can use ShiftCut locally without the MCP.

The MCP is a Verblike-hosted product that wraps:

- The ShiftCut composition agent (the LLM that authors compositions)
- Verblike's cloud rendering pipeline
- Verblike's asset libraries
...
<CardGroup cols={2}>
  <Card title="Quickstart" href="/quickstart">
    Try ShiftCut locally with the open-source CLI.
  </Card>
