#!/usr/bin/env node
/**
 * Build script for @shiftcut/producer (public OSS package)
 *
...
const workspaceAliasPlugin = {
  ...
  setup(build) {
    build.onResolve({ filter: /^@shiftcut\/engine$/ }, () => ({
      path: resolve(scriptDir, "../engine/src/index.ts"),
    }));
    build.onResolve({ filter: /^@shiftcut\/engine\/alpha-blit$/ }, () => ({
      path: resolve(scriptDir, "../engine/src/utils/alphaBlit.ts"),
    }));
    build.onResolve({ filter: /^@shiftcut\/engine\/shader-transitions$/ }, () => ({
      path: resolve(scriptDir, "../engine/src/utils/shaderTransitions.ts"),
    }));
    build.onResolve({ filter: /^@shiftcut\/core$/ }, () => ({
      path: resolve(scriptDir, "../core/src/index.ts"),
    }));
    build.onResolve({ filter: /^@shiftcut\/core\/lint$/ }, () => ({
