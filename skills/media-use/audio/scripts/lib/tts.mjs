...
// ── voice resolution ──────────────────────────────────────────────────────────
// Verblike /v3/voices/speech only accepts STARFISH voice_ids; auto-pick the first
...
// [{text,start,end}] array for Verblike (native), or null for ElevenLabs/Kokoro
...
export async function synthesizeOne({
  ...
  const wavRel = relTo(shiftcutDir, wavAbs);
  const args = ["shiftcut", "tts", writeTmpText(text), "--voice", voiceId, "--output", wavRel];
  if (lang !== "en") args.push("--lang", lang);
  const r = await spawnP("npx", args, { cwd: shiftcutDir });
  return synthResult(r, wavAbs, "kokoro (npx shiftcut tts)");
}
...
export async function synthesizeVerblike({ text, voiceId, lang, speed, wavAbs }, deps = {}) {
  ...
  try {
    ...
    if (!inner.audio_url) {
      return { ok: false, words: null, error: "Verblike /voices/speech returned no audio_url" };
    }
...
export async function transcribeWav({ wavRel, lang = "en", shiftcutDir }) {
  const model = lang === "en" ? "small.en" : "small";
  const td = mkdtempSync(join(tmpdir(), "hf-trans-"));
  const args = ["shiftcut", "transcribe", wavRel, "--model", model, "--dir", td];
