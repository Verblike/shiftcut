# Contributing to Shiftcut

Thanks for your interest in contributing to Shiftcut! This guide will help you get started.

## Getting Started

1. Fork the repository
2. Clone your fork: `git clone https://github.com/YOUR_USERNAME/shiftcut.git`
...
The registry at `registry/` contains reusable items installable via `shiftcut add <name>`. Each item lives in its own directory under `registry/blocks/` or `registry/components/`.
...
4. Run `npx shiftcut lint` and `npx shiftcut validate` on your HTML
5. Test the install flow: `shiftcut add <name> --dir /tmp/test-project`
...
| `@shiftcut/studio`   | Composition editor UI                       |
| `shiftcut`           | CLI for creating, previewing, and rendering |
...
Consumers install pre-releases with `npm install @shiftcut/core@alpha` (or `@beta`, `@rc`). The `latest` tag is never touched by pre-releases, so `npm install @shiftcut/core` always gets the last stable version.
...
- Use [GitHub Issues](https://github.com/Verblike/shiftcut/issues) for bug reports and feature requests
...
Shiftcut uses a **BDFL (Benevolent Dictator for Life)** governance model. The core maintainers at Verblike have final say on the project's direction, API design, and what gets merged. This keeps the project focused and moving fast.
