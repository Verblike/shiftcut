...
Background removal — also called *matting* in VFX — separates a foreground subject (typically a person) from its background. The output is a video with an alpha channel: fully transparent where the background was, opaque where the subject is. Drop it into any ShiftCut composition as a `<video>` tag and the subject floats over whatever you put behind them.
...
<Steps>
  <Step title="Verify ffmpeg is installed">
    ...
    Confirm with `npx shiftcut doctor` — both should be green.
  </Step>
  <Step title="Remove the background from your video">
    ```bash Terminal
    npx shiftcut remove-background subject.mp4 -o transparent.webm
    ```

    On the first run, the CLI downloads ~168 MB of model weights to `~/.cache/shiftcut/background-removal/models/`. Subsequent runs reuse the cache.
  ...
  <Step title="Drop it into a composition">
    ...
    Render the composition with the usual `shiftcut render`.
  </Step>
</Steps>
...
```bash Terminal
npx shiftcut remove-background subject.mp4 -o transparent.webm        # web playback
npx shiftcut remove-background subject.mp4 -o transparent.mov         # editing
npx shiftcut remove-background portrait.jpg -o cutout.png       # still image
```
...
```bash Terminal
npx shiftcut remove-background subject.mp4 \
  -o subject.webm \
...
Real-world numbers from the [matting eval](https://www.verblikeverse.com/a/0dd5a431-1832-4858-862d-de7fb7d02654), running u²-net_human_seg on a 4-second 1080p clip:
...
- **Force CPU on a GPU box** when you want to keep the GPU free for other work, or are debugging an EP-specific issue:
  ...
  ```bash Terminal
  npx shiftcut remove-background subject.mp4 -o transparent.webm --device cpu
  ```

- **Opt into CUDA** by setting `SHIFTCUT_CUDA=1` and providing a GPU-enabled `onnxruntime-node` build (the bundled build is CPU + CoreML only, to keep the install small for the 99% of users who don't have a GPU):

  ```bash Terminal
  SHIFTCUT_CUDA=1 npx shiftcut remove-background subject.mp4 -o transparent.webm --device cuda
  ```

Run `npx shiftcut remove-background --info` to see what providers are detected on your machine and which one `auto` would pick.
...
The CLI ships **one model on purpose** — the one that's MIT-licensed, runs everywhere, and produces production-quality output for person/portrait video. The list below leads with **free, open-source tools** that pair naturally with ShiftCut. Each entry calls out the actual catch — license, install effort, hardware needs — so you can pick the right one for your situation. Full benchmarks are in the [matting eval](https://www.verblikeverse.com/a/0dd5a431-1832-4858-862d-de7fb7d02654).
...
After running any of these externally, encode the output as a ShiftCut-compatible transparent WebM with:
...
- **Person / portrait video, web playback, MIT-clean** → use the built-in `shiftcut remove-background` (this is what it's tuned for).
...
mkdir -p ~/.cache/shiftcut/background-removal/models
curl -L -o ~/.cache/shiftcut/background-removal/models/u2net_human_seg.onnx \
...
The pipeline shells out to ffmpeg for decode + encode. Install via `brew install ffmpeg` on macOS or `sudo apt install ffmpeg` on Debian/Ubuntu. Verify with `npx shiftcut doctor`.
...
- CLI: [`shiftcut remove-background`](/packages/cli#remove-background)
- Eval: [Matting eval — v7](https://www.verblikeverse.com/a/0dd5a431-1832-4858-862d-de7fb7d02654)
