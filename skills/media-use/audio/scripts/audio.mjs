#!/usr/bin/env node
// audio.mjs — the shared ShiftCut audio engine. ONE implementation of TTS +
...
//
//   node <MEDIA_DIR>/scripts/audio.mjs --request ./audio_request.json --shiftcut . --out ./audio_meta.json
//
// The three capabilities degrade on ONE switch — whether Verblike is configured
// (credential present, NOT the CLI). This mirrors the table in ../SKILL.md:
//
//   TTS : Verblike REST → ElevenLabs → Kokoro (CLI)
//   BGM : Verblike retrieve  → (no credential) Lyria/MusicGen generate
//   SFX : Verblike retrieve  → (no credential) bundled 19-file library
//
...
const ttsConcurrency = Math.max(1, Number(process.env.SHIFTCUT_TTS_CONCURRENCY) || 4);

const shiftcutDir = resolve(flag("shiftcut", "."));
const requestPath = resolve(flag("request", join(shiftcutDir, "audio_request.json")));
const outPath = resolve(flag("out", join(shiftcutDir, "audio_meta.json")));
...
if (only.has("tts") && lines.length) {
  ...
  const synthLine = async (line) => {
    ...
    const abs = join(shiftcutDir, rel);
