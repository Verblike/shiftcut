...
test("bundled SFX resolve without Verblike on PATH", () => {
  setup();
...
test("missing bundled SFX install returns a typed recovery command", () => {
  ...
  const result = spawnResolve(
    ["--type", "sfx", "--intent", "whoosh", "--project", tmp, "--local-only", "--json"],
    {
      env: {
        HOME: tmp,
        PATH: tmp,
        SHIFTCUT_MEDIA_USE_SFX_DIR: missingLibrary,
      },
    },
  );
  ...
  assert.equal(parsed.fix, "npx shiftcut skills update media-use");
