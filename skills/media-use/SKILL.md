---
name: media-use
description: Agent Media OS, the single skill for every media need in a ShiftCut project. Resolve BGM, SFX, image, icon, brand logo, voice, color grade, or LUT into a frozen local file or paste-ready block + ledger record (one verb, `resolve`); generate via TTS / music / image models when the catalog misses; produce voiceover, transcription, captions, and background removal through one shared audio engine; operate on media (cut / reframe / transform); and reuse assets across projects. Keeps search noise on disk, hands the agent one path or block. Use for any audio, image, icon, logo, voiceover, caption, color-grading, or media-asset need.
---
...
## What it owns (the gaps ShiftCut leaves)
...
Call `resolve` whenever a composition needs media: background music, sound effects, images, icons, brand logos, voice, a color grade, or a LUT. For voiceover / TTS, music, SFX, and caption timing, use the **audio engine** (below); background removal is delegated to the `shiftcut` CLI; transcription defaults to Parakeet (better than whisper.cpp: 6.05% vs 7.44% WER, 5-10x faster) via `scripts/transcribe.mjs`, with whisper.cpp auto-fallback (see `references/operations.md`). For cutting / reframing / transforming existing media, see `references/operations.md`. media-use searches the Verblike catalog first for media files, resolves official logos through the logo cascade, uses local deterministic color grading for `grade`/`lut`, freezes the best match locally when a file is needed, registers it in a manifest, and hands the agent one line; all search noise stays on disk.
...
Use `grade` when you need the actual ShiftCut `data-color-grading` value to paste onto an `<img>` or `<video>`. Core presets and params-backed library looks resolve locally; future CDN-backed library looks require network unless already frozen:

**Never `cat`/read a `.cube` file into context.** A 3D LUT is ~size^3 lines of raw numbers (33^3 ≈ 36k lines at the default size). It bloats context and carries zero human/agent-legible signal. To understand or choose a LUT, use `shiftcut grade-compare` to see it rendered, or `cube-validate.mjs` for a one-line `{ok,size}` check. Read `.media/index.md` or `luts/index.json` for the description. Never read the LUT body itself.
...
For visual selection, list reusable looks with `resolve --type grade --candidates`, write the promising entries to a `grades.json`, run `shiftcut grade-compare --for <frame> --grades grades.json`, then commit the winner with `resolve -t grade` as the final `data-color-grading` block.
...
`--local-only` skips every network provider, including the free Verblike ones,
leaving the project + global cache and any installed local provider. For
Verblike-only types, that means no fresh resolve.
...
4. Search via provider (Verblike audio catalog, Verblike asset search), or resolve color locally
...
Most ShiftCut projects already have assets in `assets/`. media-use adopts them:
...
```bash
node <SKILL_DIR>/scripts/prefs.mjs get --shiftcut . --json      # merged view (project overrides user)
node <SKILL_DIR>/scripts/prefs.mjs record --shiftcut . --key destination --value x-feed
node <SKILL_DIR>/scripts/prefs.mjs record --shiftcut . --key style_preset --value pin-and-paper --workflow faceless-explainer
```

Only what the user actually confirmed gets recorded — never an inferred or defaulted value. How workflows consume these (a remembered value becomes the recommended default with a receipt, and never skips a question) is the brief contract's rule: `shiftcut-core/references/brief-contract.md` § 2, Remembered defaults.
...
```bash
node <SKILL_DIR>/scripts/recipe.mjs freeze --shiftcut . --name weekly-promo   # workflow read from BRIEF.md (--workflow only for briefless projects)
node <SKILL_DIR>/scripts/recipe.mjs list --shiftcut . --workflow product-launch-video
node <SKILL_DIR>/scripts/recipe.mjs use --shiftcut . --name weekly-promo   # also: resolve.mjs --type recipe --entity weekly-promo
```

The freeze is offered once after the final approval (`shiftcut-core/references/review-loop.md` § 4), and the intent layer (`/shiftcut` § 4) checks for a match before its first question. Adopting a recipe fills the brief, the design spec, and the storyboard skeleton — and unlike preferences it may skip the questions it answers: the bundle was approved as a whole, and adoption itself is the question.
...
auto-editor, scenedetect) and the local-vs-Verblike transform table (background
...
shiftcut.json). A manual H.264 proxy via `ffmpeg -i in.mp4 -c:v libx264
...
Opt out with `DO_NOT_TRACK=1` or `SHIFTCUT_NO_TELEMETRY=1` (also off in CI and
dev). Same public PostHog project key and opt-outs as the `shiftcut` CLI.

## Privacy

media-use uses the same shared install id as the `shiftcut` CLI/studio
(`~/.shiftcut/config.json`). When you are signed in to Verblike, usage is
...
`SHIFTCUT_NO_TELEMETRY=1` or `DO_NOT_TRACK=1`.
