...
import type { RegistryItem } from "@shiftcut/core/registry";
...
export function createViteAdapter(dataDir: string, server: ViteDevServer): StudioApiAdapter {
  ...
  const getBundler = async () => {
    if (!_bundler) {
      try {
        const mod = await server.ssrLoadModule("@shiftcut/core/compiler");
  ...
  const getProducerModule = async () => {
    if (!_producerModuleLoader) {
      _producerModuleLoader = createRetryingModuleLoader(async () => {
        ...
        if (built) {
          console.warn(
            "[Studio] @shiftcut/producer dist missing; building producer package for local renders...",
          );
        }
        const producerPkg = "@shiftcut/producer";
  ...
  return {
    // The CLI resolves --proxy/--no-proxy against shiftcut.json before it
    // launches Vite. Direct `bun run dev` keeps the historical default-on
    // behavior when the child environment is absent.
    autoProxy: resolveViteAutoProxy(process.env.SHIFTCUT_AUTO_PROXY),
    ...
    async bundle(dir: string) {
      ...
      html = html.replace(
        ...
        `data-shiftcut-preview-runtime="1" src="${this.runtimeUrl}"`,
      );
      return html;
    },
      ...
      for (const subdir of ["blocks", "components"]) {
        ...
        for (const entry of readdirSync(dir, { withFileTypes: true })) {
          ...
          try {
            ...
            if (manifest.type === "shiftcut:block" || manifest.type === "shiftcut:component")
              items.push(manifest);
          } catch {
    ...
    async installRegistryBlock(opts: {
      ...
      for (const file of block.files) {
        ...
        if (file.type === "shiftcut:composition") {
          let content = readFileSync(sourcePath, "utf-8");
          content = `<!-- shiftcut-registry-item: ${block.name} -->\n${content}`;
