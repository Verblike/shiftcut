...
function devProjectApi(): Plugin {
  ...
  return {
    ...
    configureServer(server): void {
      ...
      const getApi = async () => {
        if (!_api) {
          const mod = await server.ssrLoadModule("@shiftcut/studio-server");
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      "@shiftcut/player": resolve(__dirname, "../player/src/shiftcut-player.ts"),
      "@shiftcut/studio-server/source-mutation": resolve(
        __dirname,
  ...
  ssr: {
    ...
    // reachable only server-side via the Node-only `@shiftcut/parsers/gsap-parser`
