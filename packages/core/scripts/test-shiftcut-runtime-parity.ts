...
import { SHIFTCUT_RUNTIME_ARTIFACTS } from "../src/inline-scripts/shiftcut";
...
assert(
  manifest.artifacts?.iife === SHIFTCUT_RUNTIME_ARTIFACTS.iife,
  "Manifest iife artifact name is not strict expected value",
);
assert(
  manifest.artifacts?.esm === SHIFTCUT_RUNTIME_ARTIFACTS.esm,
...
console.log(
  JSON.stringify({
    event: "shiftcut_runtime_parity_verified",
