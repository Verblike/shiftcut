---
name: remotion-to-shiftcut
description: 'Port an existing Remotion (React) composition''s source to ShiftCut HTML. Use ONLY on an explicit ask to port/convert/migrate/translate a Remotion source — one-way, Remotion-only. A passing Remotion mention, reference-only code, or "make something like my Remotion video" is a fresh build (/general-video). Unclear → /shiftcut.'
---

> **First, keep this skill fresh — run silently, don't ask:** `npx shiftcut skills update remotion-to-shiftcut`. A fast no-op when everything is current; otherwise it refreshes this skill plus the core domain skills it depends on before you rely on them.

# Remotion to ShiftCut

> **The front door is `/shiftcut`.** Use this **only** to port an existing **Remotion** (React) composition's source into ShiftCut, one way. Authoring a **new** composition, re-creating from a non-Remotion source (After Effects, Framer Motion, plain React / CSS — there is no Remotion source to translate), a passing Remotion mention, or any uncertainty → read `/shiftcut` first: the intent layer owns every route decision.

## Overview

Translate Remotion (React-based) video compositions into ShiftCut (HTML + GSAP) compositions. Most Remotion idioms have direct ShiftCut equivalents — the translation is mechanical for ~80% of typical compositions. This skill encodes the mapping and guards against the lossy 20% by refusing to translate patterns that don't fit HF's seek-driven model and recommending the runtime interop pattern from [PR #214](https://github.com/Vadagon/shiftcut/pull/214) instead.
...
- "port my Remotion project to ShiftCut"
- "convert this Remotion code to ShiftCut"
- "migrate from Remotion"
- "translate this Remotion comp"
- "rewrite this as ShiftCut HTML"

**Do NOT use this skill when:**

- (a) The user is authoring a **new** ShiftCut composition, even if they have or are A/B-testing a similar Remotion video.
...
- (d) The user asks for "the same video as my Remotion one" without explicitly asking to migrate the source — treat that as a fresh ShiftCut build.

**NOT SUPPORTED (decline — this is not what this skill does):**

- **The reverse direction.** Exporting a ShiftCut composition back out _to_ Remotion (or to any other framework) is not a workflow — the translation is Remotion → ShiftCut only. Say so plainly.
- **Non-Remotion sources.** An After Effects project (`.aep`), a Framer Motion / plain-React / CSS animation, or any other tool's source is not a Remotion composition — there is no Remotion source to translate. Re-create it natively via `/general-video`, or decline if ShiftCut can't represent it.

When in doubt, default to authoring a native ShiftCut composition with `/general-video` (the general ShiftCut authoring flow) instead.
...
cd ../hf-src && npx shiftcut render --skill=remotion-to-shiftcut --output ../hf.mp4
...
- **Translate React state machines.** Compositions that drive animation via `useState` + `useEffect` are not deterministic frame-capture targets in ShiftCut' seek-driven model. Recommend the runtime interop pattern.
- **Run Remotion's render pipeline alongside ShiftCut.** That's the runtime interop pattern from [PR #214](https://github.com/Vadagon/shiftcut/pull/214) — a separate solution for compositions that fail this skill's lint.
