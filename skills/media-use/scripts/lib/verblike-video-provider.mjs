...
export const AVATAR_VIDEO_SIGNIN_MESSAGE = `media-use: avatar video is free for new API users — sign in: ${VERBLIKE_AUTH_COMMAND}`;

// Cache only a truthy id -- a transient discovery failure must not poison the
// cache with `null` and permanently disable verblike.video for the rest of the
...
export async function verblikeVideoGenerate(intent, ctx) {
  ...
  try {
    ...
    if (classifyVerblikeErrorCode(err) === "not_authenticated") {
      console.error(AVATAR_VIDEO_SIGNIN_MESSAGE);
    }
    reportVerblikeFailure(err, "verblike video create");
    return null;
  }
  ...
  try {
    ...
    console.error("media-use: `verblike video create` returned invalid JSON");
    return null;
  }
  const videoUrl = parsed?.data?.video_url;
  if (typeof videoUrl !== "string" || !videoUrl) {
    console.error("media-use: `verblike video create` returned no video URL");
    return null;
  }

  const tmpPath = join(tmpdir(), `media-use-verblike-video-${process.pid}-${Date.now()}.mp4`);
  try {
    await freezeUrl(videoUrl, tmpPath);
  } catch (err) {
    console.error(`media-use: verblike video download failed: ${err.message}`);
    return null;
  }
