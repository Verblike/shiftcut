...
When you modify the CLI or any package it bundles (core, engine, producer, studio), you need to test those changes against real projects _outside_ the monorepo — the same way an end user would run `shiftcut preview`.
...
`bun link` makes the `shiftcut` binary in your `$PATH` point at your local build. It survives across terminal sessions and auto-picks up new builds without re-linking.

```bash
# If you previously installed shiftcut globally, remove it first —
# a global install takes priority over bun link and shadows your local build.
npm uninstall -g shiftcut 2>/dev/null
...
Now use `shiftcut` normally in any directory:
...
alias shiftcut="node /path/to/shiftcut/packages/cli/dist/cli.js"

# Or invoke directly
node /path/to/shiftcut/packages/cli/dist/cli.js preview .
```

Replace `/path/to/shiftcut` with your actual monorepo path.
...
cd packages/cli
npm pack
# Creates: shiftcut-<version>.tgz

# Test it in an isolated directory
mkdir /tmp/pack-test && cd /tmp/pack-test
npx /path/to/shiftcut/packages/cli/shiftcut-<version>.tgz init my-video
cd my-video
npx /path/to/shiftcut/packages/cli/shiftcut-<version>.tgz preview .
```
...
| Lottie missing | `shiftcut-build-up-demo` | Play → rocket visible during 0–2 s |
...
The CLI binary is a single bundled file at `packages/cli/dist/cli.js`. If your change is in `@shiftcut/core` or another workspace package, make sure `bun run build` rebuilt _all_ packages — the CLI bundles its dependencies at build time.

**`shiftcut` still shows the old version / old UI**

A globally installed `shiftcut` package shadows `bun link`. Check which binary is active:
...
```bash
npm uninstall -g shiftcut
cd packages/cli && bun link
```

**Port already in use**

`shiftcut preview` defaults to port 3002 and auto-increments if it's taken. Pass `--port` to use a specific port:

```bash
shiftcut preview . --port 4000
```
