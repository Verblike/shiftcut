/**
 * Embedded studio server for `shiftcut preview` outside the monorepo.
 *
 * Uses the shared studio API module from @shiftcut/core/studio-api,
...
} from "@shiftcut/studio-server";
import { resolveAutoProxy } from "../utils/projectConfig.js";
import { getElementScreenshotClip } from "@shiftcut/studio-server/screenshot-clip";
import type { ScreenshotClip } from "@shiftcut/studio-server/screenshot-clip";
import type { RenderJob } from "@shiftcut/producer";

const STUDIO_MANUAL_EDITS_PATH = ".shiftcut/studio-manual-edits.json";
...
async function loadStudioProducer() {
  return isDevMode()
    ...
    : await import("@shiftcut/producer");
}
...
async function getThumbnailBrowser(): Promise<import("puppeteer-core").Browser | null> {
  ...
  _thumbnailBrowserInitializing = (async () => {
    try {
      ...
      const { acquireBrowser, buildChromeArgs } = await import("@shiftcut/engine");
...
export interface StudioServerOptions {
   ...
   * `shiftcut.json` value; when omitted, the project's `media.autoProxy`
...
export function createStudioServer(options: StudioServerOptions): StudioServer {
  ...
  const adapter: PreviewApiAdapter = {
    ...
    // otherwise honor the project's shiftcut.json media.autoProxy so every
    ...
    async bundle(dir: string): Promise<string | null> {
      try {
        const { bundleToSingleHtml } = await import("@shiftcut/core/compiler");
        ...
        html = html.replace(
          'data-shiftcut-preview-runtime="1" src=""',
          'data-shiftcut-preview-runtime="1" src="/api/runtime.js"',
        );
        return html;
      } catch (err) {
    ...
    async transformPreviewHtml({ html, project }) {
      ...
      const gifOutputDir = join(project.dir, ".shiftcut", "prepared-assets", "gif");
      const gifDownloadDir = join(project.dir, ".shiftcut", "prepared-assets", "downloads");
      const prepared = await prepareAnimatedGifInputs(html, {
        projectDir: project.dir,
        downloadDir: gifDownloadDir,
        outputDir: gifOutputDir,
        outputSrcPrefix: ".shiftcut/prepared-assets/gif",
    ...
    async listRegistryCatalog() {
      ...
      const blockAndComponentEntries = entries.filter(
        (e) => e.type === "shiftcut:block" || e.type === "shiftcut:component",
      );
    ...
    async installRegistryBlock(opts) {
      ...
      // `shiftcut add`), so an incompatible block or dep aborts cleanly.
      const warnings = gateRegistryItemsCompatibility(items);
      for (const warning of warnings) {
        process.stderr.write(`shiftcut:registry ${warning}\n`);
      }
  ...
  // ShiftCut instances and reuse them instead of spawning duplicates.
  // See portUtils.ts detectShiftcutServer() for the consumer.
  app.get("/__shiftcut_config", (c) => {
    const serve = async () => {
...
<html>
  <head>
    ...
    <title>ShiftCut Studio unavailable</title>
    <style>
      body {
        margin: 0;
        min-height: 100vh;
