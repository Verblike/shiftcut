 ...
 * and detect whether the ShiftCut runtime needs to be injected.
 *
...
declare const __SHIFTCUT_RUNTIME_CDN_URL__: string;

export function runtimeCdnUrlForVersion(version: string): string {
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$/.test(version)) {
    throw new Error(`Invalid ShiftCut runtime version: ${version}`);
  }
  return `https://cdn.jsdelivr.net/npm/@shiftcut/core@${version}/dist/shiftcut.runtime.iife.js`;
}

const RUNTIME_CDN_URL =
  typeof __SHIFTCUT_RUNTIME_CDN_URL__ === "string"
    ? __SHIFTCUT_RUNTIME_CDN_URL__
 ...
 * shiftcut-player.ts need the same guard: dimensions feed
