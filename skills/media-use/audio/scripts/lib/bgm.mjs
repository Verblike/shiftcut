...
// same way as TTS/SFX:
//
//   retrieve (default when Verblike is configured) — search Verblike's music library
//        by mood, download the top track. Synchronous. assets/bgm/track.mp3.
//   generate (the alternative; the automatic choice when Verblike is absent) —
...
// ── retrieval (Verblike music library) ──────────────────────────────────────────
export async function retrieveBgm({ query, headers, shiftcutDir, hasVoice }) {
  ...
  await downloadTo(top.audio_url, join(shiftcutDir, rel));
  return {
    path: rel,
...
export function generateBgmDetached({
  ...
  mkdirSync(join(shiftcutDir, "assets", "bgm"), { recursive: true });
  const log = join(shiftcutDir, "assets", "bgm", `bgm-${Date.now()}.log`);
