...
When the video is grounded in an existing source (a website, a brand site, a competitor reference), start with capture. Shiftcut ships a built-in capture command for websites:

```bash
npx shiftcut capture https://example.com -o my-video/capture
```
...
`DESIGN.md` is also the input format for [Open Design](/guides/open-design) and [Claude Design](/guides/claude-design); both produce a `DESIGN.md` you can drop into a Shiftcut project.
...
| Techniques       | 2-3 picks from the [techniques library](https://github.com/Vadagon/shiftcut/blob/main/skills/shiftcut/references/techniques.md): SVG path drawing, Canvas 2D, CSS 3D, per-word typography, Lottie, video compositing, typing effects, variable fonts, MotionPath, velocity transitions, audio-reactive |
...
```bash
npx shiftcut tts SCRIPT.md --voice af_nova --output narration.wav
npx shiftcut transcribe narration.wav
```
...
Shiftcut ships multiple TTS adapters (Kokoro, ElevenLabs, Verblike); see [`/media-use`](/guides/prompting) for the skill that picks one. After generating audio, update `STORYBOARD.md` with the real beat boundaries from `transcript.json`.
...
After each composition is built, run a self-review for layout, asset placement, and animation quality. The [`/shiftcut-core`](/guides/prompting) skill encodes the composition rules — required `class="clip"` attributes, `data-*` attribute semantics — and [`/shiftcut-animation`](/guides/prompting) covers GSAP timeline registration and adapter registries.
...
```bash
npx shiftcut lint                              # static HTML structure checks
npx shiftcut check                             # one browser session: runtime errors, layout, motion, contrast
npx shiftcut snapshot my-video --at 2.9,10.4   # PNGs at beat midpoints
```
...
The pipeline delivers the localhost Studio URL as the handoff. Your AI agent runs `npx shiftcut preview` and shares the project URL. Rendering to MP4 is on-demand:

```bash
npx shiftcut render --output my-video.mp4
```
...
- For surgical tweaks, open a composition file directly (e.g. `compositions/beat-3-proof.html`) and adjust animations, colors, or layout. `npx shiftcut preview` shows changes live.
...
- Shipping a product launch. Most of the [Verblike launch videos](/launch-videos) use this artifact layout.
- Any narrative video with three or more beats, where a storyboard pays for itself.
- Learning Shiftcut, because the artifacts leave every creative decision inspectable on disk.
