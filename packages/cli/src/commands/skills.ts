...
export const examples: Example[] = [
  ["Install all ShiftCut skills", "shiftcut skills"],
  ["Check whether installed skills are up to date", "shiftcut skills check"],
  ["Check, machine-readable (for agents / CI)", "shiftcut skills check --json"],
  ["Update the core set + everything already installed", "shiftcut skills update"],
  ["Also install one workflow (on-demand install)", "shiftcut skills update pr-to-video"],
];
...
function mirrorToInstalledAgents(): void {
  try {
    const names = shiftcutSkillNames({ scope: "global" });
...
export async function updateSkills(
  ...
  try {
    ...
    // stale shiftcut checkout could resolve "latest" from that stale local
...
function renderCheck(result: SkillsCheckResult): void {
  ...
  console.log();
  console.log(c.bold("shiftcut skills"));
  console.log();

  if (!result.location) {
    console.log(`  ${c.dim("No ShiftCut skills found in the usual locations.")}`);
    console.log(`  ${c.accent("Install: npx shiftcut skills")}`);
    console.log();
    return;
  }
  ...
  if (result.updateAvailable) {
    console.log(`  ${c.accent("Update: npx shiftcut skills update")}`);
  } else {
...
const checkCommand = defineCommand({
  ...
  async run({ args }) {
    ...
    //   shiftcut skills check || npx shiftcut skills update
...
const updateCommand = defineCommand({
  meta: {
    name: "update",
    description:
      "Update the core set plus every installed ShiftCut skill to the latest, and remove any no longer published. Pass skill names to also install those (how workflow skills install on demand) — without names it never expands a partial install",
  },
  ...
  async run({ args }) {
    ...
    // Positional skill names (e.g. `shiftcut skills update pr-to-video`) are
    ...
    // /shiftcut router runs it after picking a workflow, before reading the
    // workflow's skill.
    ...
    // ShiftCut repo so `update` reliably refreshes the published skills.
    //
    // strict: this is the documented recovery path for the agent/CI contract
    // `shiftcut skills check || shiftcut skills update`, and the router's
...
export default defineCommand({
  meta: {
    name: "skills",
    description: "Install, check, and update ShiftCut skills for AI coding tools",
  },
  subCommands: {
    check: checkCommand,
  ...
  async run({ args }) {
    ...
    // the positional so bare `shiftcut skills` installs, while
    // `shiftcut skills check|update` does not also re-install.
    if (!args._?.[0]) {
      await installSkills("*");
