...
description: How to add blocks and components to the ShiftCut registry.
---

Your agent already knows how to build video components. It writes HTML. ShiftCut renders it. The registry is the collection of everything that's been built — 52 blocks and counting.

This guide shows you how to add to it.

<Info>
**Quick version** — Fork the repo. Write one HTML file with a paused GSAP timeline. Add `registry-item.json`. Run `shiftcut lint` + `validate`. Publish with `npx shiftcut publish`. Open a PR.
</Info>

## Why Contribute?

Every block in the registry exists because someone needed it and built it. When you add a block, every ShiftCut user gets it with one command:

```bash
npx shiftcut add instagram-follow
```

The registry grows, ShiftCut gets more useful, and your work ships to everyone.
...
Open an issue on [GitHub](https://github.com/Vadagon/shiftcut/issues) with a visual reference. Tag it `component-request`.
...
If you use Claude Code with ShiftCut skills:

> "I want to contribute a new transition that looks like \[description\]"

The `/shiftcut-registry` skill scaffolds the structure, validates, renders a preview, publishes to [shiftcut.verblike.com](https://shiftcut.verblike.com), and prepares the PR.
...
```json
{
  "$schema": "https://shiftcut.verblike.com/schema/registry-item.json",
  "name": "my-block",
  "type": "shiftcut:block",
  "title": "My Block",
...
<Steps>
  <Step title="Fork and create">
    Fork [Vadagon/shiftcut](https://github.com/Vadagon/shiftcut) and create your block directory:
    ```bash
  ...
  <Step title="Render preview">
    ```bash
    shiftcut render -o preview.mp4
    ```
  </Step>
  <Step title="Publish and PR">
    ```bash
    npx shiftcut publish
    ```
    Open a PR with your [shiftcut.verblike.com](https://shiftcut.verblike.com) preview link.
  </Step>
</Steps>

**External contributors:** attach the preview MP4 to your PR. A maintainer handles the catalog image.

**Verblike internal:** run `scripts/upload-docs-images.sh` to push catalog PNGs.
