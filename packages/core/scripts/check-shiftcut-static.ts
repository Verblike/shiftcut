...
function formatCounts(result: ShiftCutLintResult): string {
...
function formatHumanOutput(result: ShiftCutLintResult, resolvedPath: string): string {
...
async function main() {
  ...
  if (!inputPath) {
    console.error(
      "Usage: bun run check:shiftcut-html [--json] <path-to-html>\nExample: bun run check:shiftcut-html core/src/tests/broken-video.html",
    );
    process.exit(2);
  }
  ...
  const result = await lintShiftCutHtml(html, { filePath: resolvedPath });
