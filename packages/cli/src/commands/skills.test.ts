...
const DEFAULT_CHECK = {
  ...
  skills: [
    { name: "shiftcut", status: "outdated" },
    { name: "shiftcut-core", status: "missing" },
...
vi.mock("../utils/skillsManifest.js", async (importOriginal) => {
  ...
  return {
    ...
    shiftcutSkillNames: vi.fn(() => ["shiftcut"]),
...
// dev machine's real ~/.shiftcut config; the invalidation behavior itself
...
describe("shiftcut skills", () => {
  ...
  it("skills update refreshes only the stale core + installed skills — never the full set", async () => {
    ...
    expect(skillFlagValues(args).sort()).toEqual(["shiftcut", "shiftcut-core"]);
  ...
  // `targets` (isCoreSkill pattern-matches `shiftcut-*`), `skills add`
  ...
  it("self-heals an orphaned lock entry after `skills remove` no-ops on it", async () => {
    ...
    vi.mocked(checkSkills)
      ...
      .mockResolvedValueOnce({
        scope: "global",
        skills: [{ name: "shiftcut-captions", status: "removed" }],
      } as never);
    vi.mocked(pruneOrphanedLockEntries).mockReturnValueOnce(["shiftcut-captions"]);

    await runSkillsUpdate();

    expect(pruneOrphanedLockEntries).toHaveBeenCalledWith(["shiftcut-captions"], "global");
    expect(process.exitCode).toBe(0);
  });
...
// The router contract: `/shiftcut` picks a workflow, then runs
// `shiftcut skills update <workflow>` so the workflow's skill (and the core
...
describe("shiftcut skills update <names>", () => {
  let prevExitCode: typeof process.exitCode;
  ...
  it("is a fast no-op (no install spawn) when everything is already current", async () => {
    ...
    vi.mocked(checkSkills).mockResolvedValue({
      ...
      skills: [
        { name: "shiftcut", status: "current" },
        { name: "shiftcut-core", status: "current" },
