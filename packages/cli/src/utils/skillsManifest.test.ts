...
describe("isCoreSkill", () => {
  it("classifies the entry router, shiftcut-* domain skills, and media-use as core", () => {
    expect(isCoreSkill("shiftcut")).toBe(true);
    expect(isCoreSkill("shiftcut-core")).toBe(true);
    expect(isCoreSkill("shiftcut-animation")).toBe(true);
...
describe(".claude-plugin/marketplace.json core-skills pin", () => {
  ...
  // entry (not plugin.json, and not the `shiftcut` entry) precisely so
  // the full `shiftcut` plugin keeps auto-discovering all skills.
  it("lists exactly the core skills present in the repo's skills/ tree", () => {
    ...
    const full = marketplace.plugins?.find((p) => p.name === "shiftcut");
...
describe("presentSkills", () => {
  it("returns only the names present in the located install", () => {
    ...
    mkdirSync(join(skillsDir, "shiftcut"), { recursive: true });
    writeFileSync(join(skillsDir, "shiftcut", "SKILL.md"), "# shiftcut");

    expect(presentSkills(["shiftcut", "pr-to-video"], { cwd: project, home })).toEqual([
      "shiftcut",
    ]);
  });
  ...
  it("returns [] when no install exists at all", () => {
    ...
    mkdirSync(project, { recursive: true });
    expect(presentSkills(["shiftcut"], { cwd: project, home })).toEqual([]);
  });
});
...
describe("checkSkills install detection", () => {
  ...
  it("prefers global scope over project (matches how agents load skills)", async () => {
    ...
    // the project scope, and ShiftCut installs globally — so check reports on
  ...
  it("reports no location and an available update when nothing is installed", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        source: "test",
        skills: { shiftcut: { hash: "x", files: 1 }, alpha: { hash: "y", files: 1 } },
      }),
    );
