...
<html><body>
  ...
  <script>
    ...
    const runtimeBlock = bundled.match(
      /<script\b[^>]*data-shiftcut-preview-runtime[^>]*>[\s\S]*?<\/script>/i,
    )?.[0];
    ...
    expect(runtimeBlock).toMatch(/data-shiftcut-preview-runtime="1">/);
...
<html data-composition-variables='[
  ...
  <body>
    <div id="card-root" data-composition-id="card" data-width="1920" data-height="1080">
      <script>
        window.__timelines = window.__timelines || {};
        window.__timelines[document.currentScript?.dataset.slot || "missing"] = __shiftcut.getVariables();
      </script>
    </div>
  </body>
</html>`,
...
<html><head>
  ...
  <div data-composition-id="scene" data-width="1920" data-height="1080">
    ...
    expect(bundled).toContain("(function(document, gsap, window, __shiftcut)");
...
<body>
  <div data-composition-id="root" data-width="640" data-height="360">
    ...
    const bundled = await bundleToSingleHtml(dir);
    const { document } = parseHTML(bundled);
    const styleEls = document.querySelectorAll("style[data-shiftcut-text-rendering]");
