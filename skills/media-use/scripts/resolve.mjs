...
async function run() {
  ...
  // Offline guard: --local-only skips every remote provider (Verblike catalog),
  ...
  if (!searchResult) {
    ...
    // brand stays local: no frame.md/design.md -> upsell the ShiftCut design
    // flow rather than reporting a generic miss (B5).
    const msg =
      providerFailure instanceof BundledSfxAssetsError
        ? providerFailure.message
        : type === "brand"
          ? "no brand spec found — add a frame.md or design.md (colors/font/logo) to this project. Run the ShiftCut design flow to create one; brand tokens are read locally for deterministic rendering."
          : args.provider
  ...
  // reusable across all shiftcut projects (B3). Non-fatal; dedup by sha.
