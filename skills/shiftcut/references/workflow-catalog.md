...
- **Input:** A brief, outline, or existing page to author as a presentation, pitch deck, or interactive deck. If “slides”, “deck”, or “convert this page” is ambiguous, confirm that the user wants a ShiftCut slideshow before authoring.
- **Output:** A runnable ShiftCut composition plus the JSON island used by `SlideshowController`: discrete slides, fragment reveals, branching, hotspots, presenter mode, and speaker notes. The deliverable is a navigable deck, not an MP4.
...
- **Output:** A ShiftCut composition of any length or format through design → plan → static layout → animation → check → approval → render.
...
- **Output:** A ShiftCut HTML composition translated from the source and compared with the Remotion render through the migration evaluation harness.
