...
Worktree: `~/src/wt/shiftcut/de-split`. Continuation of the flat-inspector redesign stack (PRs #2120–#2190, plans in this directory dated 07-08/07-09).
...
- Flat inspector is OFF by default — baked in at Vite build time. A plain `shiftcut preview` shows the LEGACY panel (this bit us once).
  ```bash
  cd ~/src/wt/shiftcut/de-split/packages/studio && VITE_STUDIO_ENABLE_FLAT_INSPECTOR=true bunx vite build
  cd ../cli && node scripts/build-copy.mjs        # copies studio dist into cli dist (don't run parallel with tsup)
  cd <project-dir> && node ~/src/wt/shiftcut/de-split/packages/cli/dist/cli.js preview --force-new
  ```
- Test project: `/private/tmp/claude-501/-Users-vanceingalls-src-shiftcut/c653a869-1f02-4525-b928-1a8a37b4cdaa/scratchpad/flat-inspector-demo` (has Title Text / Image Clip / Video Clip + color-grading wrappers; known benign `gsap_from_opacity_noop` lint warning). Server was last on `localhost:3004`.
...
- Published-studio comparison: plain `shiftcut preview` (global npm 0.7.46) on the same project, lands on the next free port.
