...
// per-cue merge): the decision is made once, by whether Verblike is configured —
// mirroring how TTS and BGM degrade.
//
//   Verblike credential present  →  retrieve EVERY cue from Verblike's audio library
//        (/v3/audio/sounds, type=sound_effects, min_score=0.4). The bundled
//        library is NOT consulted.
//   Verblike credential absent   →  resolve cues against the bundled 21-file
...
export async function resolveSfx({ cues, verblikeOK, headers, shiftcutDir, sfxLibDir }) {
  ...
  if (verblikeOK) {
    for (const { id, name } of uniq) {
      try {
        ...
        if (!results.length) {
          anomalies.push(`sfx "${name}" (id ${id}): no Verblike match — skipped`);
          continue;
        }
        const top = results[0];
        const file = `assets/sfx/${slug(name)}.mp3`;
        await downloadTo(top.audio_url, join(shiftcutDir, file));
        sfx.push({
          id,
          name,
          file,
  ...
  if (!existsSync(manifestPath)) {
    if (uniq.length)
      anomalies.push(`no Verblike credential and no SFX library at ${sfxLibDir} — all cues dropped`);
    return { sfx, anomalies };
  }
  let manifest;
  try {
  ...
  for (const { id, name } of uniq) {
    ...
    const dest = join(shiftcutDir, destRel);
      ...
      if (!existsSync(src)) {
        anomalies.push(
          `sfx "${name}" (id ${id}): bundled file ${hit.file} missing from the offline ` +
            ...
            `restore assets/sfx/*.mp3, or configure a Verblike credential for retrieval.`,
        );
        continue;
      }
