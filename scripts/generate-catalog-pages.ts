...
function discoverItems(): { kind: ItemKind; manifest: RegistryItem }[] {
  ...
  for (const item of registryManifest.items ?? []) {
    const kind =
      item.type === "shiftcut:block"
        ? "block"
        : item.type === "shiftcut:component"
          ? "component"
          : null;
...
function typeDir(kind: ItemKind): string {
  return ITEM_TYPE_DIRS[kind === "block" ? "shiftcut:block" : "shiftcut:component"];
}
...
function generateTextureAgentUsage(
  ...
  return [
    ...
    `   npx shiftcut add ${manifest.name}`,
    ...
    "```",
    "",
    `After install, the snippet lives at \`${installedSnippet}\` inside the project where you ran \`npx shiftcut add ${manifest.name}\`. The part to paste is the real \`<style>\` element near the bottom of that file; the texture PNGs install to \`assets/${manifest.name}/masks/\` and are referenced by project-root URLs in that CSS.`,
    "",
...
function generateItemMdx(kind: ItemKind, manifest: RegistryItem): string {
  ...
  const installCmd = `npx shiftcut add ${manifest.name}`;
  ...
  const primaryFile =
    manifest.files.find((f) => f.type === "shiftcut:composition") ??
    manifest.files.find((f) => f.type === "shiftcut:snippet") ??
    manifest.files[0];
