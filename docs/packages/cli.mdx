...
The `shiftcut` CLI is the primary way to work with Shiftcut. It handles project creation, live preview, rendering, linting, and diagnostics — all from your terminal.
...
<Tip>
  The CLI is the recommended starting point for all Shiftcut users. It wraps the producer, engine, and studio packages so you do not need to install them separately.
</Tip>
...
<Tabs>
  <Tab title="Agent mode (default)">
    ...
    npx shiftcut init my-video --example blank --video video.mp4
    npx shiftcut render --output output.mp4 --fps 30 --quality standard
    npx shiftcut upgrade --check --json
    ```
  </Tab>
...
```
  ...
  Run: npx shiftcut@latest
```

This is suppressed in CI environments, non-TTY shells, and when `SHIFTCUT_NO_UPDATE_CHECK=1` is set.
...
<Steps>
  <Step title="Create a project">
    ...
    ```bash
    npx shiftcut init --example warm-grain
    ```
    You will be prompted for a project name, or pass it as an argument:
    ```bash
    npx shiftcut init my-video --example warm-grain
    ```
  ...
  <Step title="Preview in browser">
    ...
    ```
    The Shiftcut Studio opens in your browser. Edit `index.html` and the preview updates instantly.
  </Step>
  ...
  <Step title="Render to MP4">
    ...
    ```bash
    npx shiftcut render --output output.mp4
    ```
    Render a specific composition instead of `index.html`:
    ```bash
    npx shiftcut render -c compositions/intro.html -o intro.mp4
    ```
    For deterministic output, add `--docker`:
    ```bash
    npx shiftcut render --docker --output output.mp4
    ```
  </Step>
</Steps>
...
<Tabs>
  <Tab title="Create">
    ...
    npx shiftcut init my-video --example blank --tailwind
    ...
    npx shiftcut add claude-code-window

    # Add a component (effect / snippet)
    npx shiftcut add shader-wipe

    # Target a different project dir
    npx shiftcut add shader-wipe --dir ./my-video

    # Headless / CI (skip clipboard; also: --json for a machine-readable result)
    npx shiftcut add shader-wipe --no-clipboard --json
    ```
    ...
    `add` reads [`shiftcut.json`](#shiftcut-json) at the project root to know which registry to pull from and where to drop files. If the file is missing but the directory looks like a Shiftcut project (has `index.html`), a default `shiftcut.json` is written the first time you run `add`.
    ...
    Trying `add` with an example's name (e.g. `shiftcut add warm-grain`) emits a clear error pointing you at `init --example`.
    ...
    npx shiftcut transcribe audio.mp3
    npx shiftcut transcribe video.mp4 --model medium.en --language en

    # Import existing transcripts from other tools
    npx shiftcut transcribe subtitles.srt
    npx shiftcut transcribe captions.vtt
    npx shiftcut transcribe openai-response.json

    # Export caption sidecars from transcript.json
    npx shiftcut transcribe transcript.json --to srt
    ```
    ...
    | `--timeout` | Whisper spawn timeout in ms. Overrides the duration+model auto-scaled default. Increase on slow CPUs (e.g. emulated arm64/x64) where medium/large models take many seconds per audio second. Minimum 5000. Env: `SHIFTCUT_TRANSCRIBE_TIMEOUT_MS`. |
    ...
    **Supported transcript formats:**
    ...
    | Format | Source |
    |--------|--------|
    | whisper.cpp JSON | `shiftcut init --video`, `shiftcut transcribe` |
...
Word-level transcripts (whisper output) are grouped into readable caption cues on sentence boundaries. Exporting directly from an `.srt`/`.vtt` source keeps its cue boundaries unchanged. When exporting from a `transcript.json` whose entries are already finished cues with no internal spaces (single-word or CJK captions), pass `--preserve-cues` to keep them one cue per entry.
    ...
    npx shiftcut tts "Welcome to ShiftCut"

    # Choose a voice
    npx shiftcut tts "Hello world" --voice am_adam

    # Save to a specific file
    npx shiftcut tts "Intro" --voice bf_emma --output narration.wav

    # Adjust speech speed
    npx shiftcut tts "Slow and clear" --speed 0.8

    # Generate Spanish speech (lang auto-detected from the `e` voice prefix)
    npx shiftcut tts "La reunión empieza a las nueve" --voice ef_dora --output es.wav

    # Override the phonemizer (read English text with a French voice)
    npx shiftcut tts "Bonjour le monde" --voice af_heart --lang fr-fr

    # Read text from a file
    npx shiftcut tts script.txt

    # List available voices
    npx shiftcut tts --list
    ```
    ...
    npx shiftcut remove-background avatar.mp4 -o transparent.webm

    # ProRes 4444 .mov for editing round-trip
    npx shiftcut remove-background avatar.mp4 -o transparent.mov

    # Single image → transparent PNG
    npx shiftcut remove-background portrait.jpg -o cutout.png

    # Layer separation: cutout AND inverse-alpha background plate in one pass
    npx shiftcut remove-background avatar.mp4 \
      -o subject.webm --background-output plate.webm

    # Force CPU on a machine that has CoreML or CUDA
    npx shiftcut remove-background avatar.mp4 -o transparent.webm --device cpu

    # Inspect detected providers without rendering
    npx shiftcut remove-background --info
    ```
    ...
    The model is `u2net_human_seg` (MIT, ~168 MB ONNX). Weights download to `~/.cache/shiftcut/background-removal/models/` on first run and are reused thereafter. Peak inference RAM is ~1.5 GB.

    `--device auto` picks CoreML on Apple Silicon, CUDA when available, and CPU otherwise. The CLI bundles the CPU build of `onnxruntime-node`; for CUDA, set `SHIFTCUT_CUDA=1` and provide a GPU-enabled `onnxruntime-node` build.
    ...
    ```bash
    npx shiftcut capture https://stripe.com
    npx shiftcut capture https://linear.app -o linear-capture
    npx shiftcut capture https://example.com --json
    ```
  ...
  <Tab title="Preview">
    ...
    Opens your composition in the Shiftcut Studio with live preview. Edits to `index.html` and any referenced sub-compositions are reflected automatically. The preview uses the same Shiftcut runtime as production rendering, so what you see is what you get.
    ...
    2. **Local studio mode** — if `@shiftcut/studio` is installed in your project's `node_modules`, spawns Vite with full HMR for faster iteration.
    3. **Monorepo mode** — if running from the Shiftcut source repo, spawns the studio dev server directly.

    ### `publish`

    Upload the project and get back a stable `shiftcut.verblike.com` URL:

    ```bash
    npx shiftcut publish [dir]
    npx shiftcut publish --yes
    ```
    ...
    `publish` zips the current project, uploads it to the ShiftCut publish backend, and prints a stable `shiftcut.verblike.com` URL for that stored project.

    The printed URL already includes the claim token, so opening it on `shiftcut.verblike.com` lets the intended user claim the uploaded project and continue editing in the web app.

    This flow does not keep a local preview server alive and does not open a tunnel. The published URL resolves to the persisted project stored by Verblike, so it keeps working after the CLI process exits.
    ...
    ```bash
    npx shiftcut lint [dir]
    npx shiftcut lint [dir] --verbose   # include info-level findings
    npx shiftcut lint [dir] --json      # machine-readable JSON output
    ```
    ```
    ◆  Linting my-project/index.html
    ...
    **Severity levels:**
    ...
    ```bash
    npx shiftcut check [dir]
    npx shiftcut check [dir] --json         # {ok, lint, runtime, layout, motion, contrast, snapshots}
    npx shiftcut check [dir] --snapshots    # annotated overview frames + per-finding crops
    npx shiftcut check [dir] --at 1.5,4,7.25
    npx shiftcut check [dir] --strict       # exit non-zero on warnings too
    ```
    ...
    ```bash
    npx shiftcut beats [dir]
    npx shiftcut beats [dir] --json   # machine-readable JSON output
    ```
    ...
    Requires a local Chrome (the same one used by `render`; run `npx shiftcut browser ensure` if missing). Detection runs the **same** algorithm the Studio uses; results are near-identical (a different headless-Chrome audio sample rate can shift beat times by a frame or two).
    ...
    ```bash
    npx shiftcut inspect [dir]
    npx shiftcut inspect [dir] --json
    npx shiftcut inspect [dir] --samples 15
    npx shiftcut inspect [dir] --at 1.5,4,7.25
    ```
    ...
    ```bash
    npx shiftcut snapshot my-project --at 2.9,10.4,18.7
    npx shiftcut snapshot my-project --frames 10
    ```
  ...
  <Tab title="Build">
    ...
    npx shiftcut render --format webm --output overlay.webm

    # With options
    npx shiftcut render --output output.mp4 --fps 60 --quality high

    # Opt out of local browser GPU capture
    npx shiftcut render --no-browser-gpu --output cpu-browser.mp4

    # Add hardware FFmpeg encoding
    npx shiftcut render --gpu --output gpu.mp4
    ```
    ...
    | `--variables` | JSON object | — | Variable overrides merged over `data-composition-variables` defaults. Read via `window.__shiftcut.getVariables()` |
    ...
    | `--frames-cache-dir` | path or `off` / `none` / `false` / `0` | `<tmpdir>/shiftcut-extract-cache-<uid>` | Directory for the content-addressed extracted-frame cache. Relocate it off the system drive when the OS temp directory lives on a small partition — long renders can accumulate multi-GB of frames, and on Windows `%TEMP%` defaults to `C:` (reporter `ts=1784219488`, CLI 0.7.58, 15 GB laptop: extract cache exhausted C: mid-render). Pass an opt-out alias (`off`, `none`, `false`, `0`) to disable caching entirely; frames then extract into the render's `workDir` and are cleaned up when the render ends. Env fallback `SHIFTCUT_EXTRACT_CACHE_DIR`. `shiftcut doctor` reports the effective directory + free space at that location. |
    ...
    <html
      ...
      <body>
        ...
        <script>
          const vars = window.__shiftcut.getVariables();
    ...
    npx shiftcut render --output default.mp4

    # Override at render time — missing keys fall through to declared defaults
    npx shiftcut render --variables '{"title":"Q4 Report","theme":"dark"}' --output q4.mp4

    # Pass values from a JSON file
    npx shiftcut render --variables-file ./vars.json --output out.mp4
    ```
    ...
    npx shiftcut render --format webm --output captions.webm
  ...
  <Tab title="Utilities">
    ...
    **CI gating.** `shiftcut doctor --json` always exits 0 on successful execution — the command succeeded if it produced valid output. Whether the environment is healthy is carried in the `ok` field of the payload, so a new CLI release (which flips `Version.ok` to `false`) never breaks your pipeline. Pipe through `jq` to gate on the payload instead:
    ...
    npx shiftcut upgrade --check         # check and exit (no prompt)
    npx shiftcut upgrade --check --json  # machine-readable for agents
    npx shiftcut upgrade --yes           # show upgrade commands without prompting
    ```
    ...
    npx shiftcut browser path

    # Remove cached Chrome download
    npx shiftcut browser clear
    ```

    The `path` subcommand outputs only the path, useful in scripts: `$(npx shiftcut browser path)`.
    ...
    npx shiftcut feedback --rating 10

    # Rating with optional details
    npx shiftcut feedback --rating 7 --comment "render succeeded but GSAP timeline didn't animate"

    # Also file a pre-filled GitHub issue with a published minimal repro (opt-in, consented)
    npx shiftcut feedback --rating 3 --comment "GSAP timeline froze on seek" --file-issue
    ```
    ...
    Install ShiftCut skills for AI coding tools, including first-party runtime adapter skills:
    ...
    npx shiftcut skills --claude
    npx shiftcut skills --cursor
    npx shiftcut skills --claude --gemini
    ```
    ...
    Skills are fetched from GitHub: the `/shiftcut` entry skill (routes "make me a video" to a workflow), the composition contract and Tailwind v4 browser-runtime guidance (`/shiftcut-core`), all animation including the GSAP / Anime.js / CSS / Lottie / Three.js / WAAPI / TypeGPU runtime adapters (`/shiftcut-animation`), creative direction (`/shiftcut-creative`), media preprocessing (`/media-use`), registry block/component wiring (`/shiftcut-registry`), and the video workflows. The `init` command also offers to install skills automatically after scaffolding a project.
    ...
    **Using `shiftcut skills` is already fine** — as of v0.4.5 the CLI sets `GIT_CLONE_PROTECTION_ACTIVE=0` on the child environment, which is the opt-in knob Git provides for exactly this case. You don't need to do anything.

    **If you ran `npx skills add Verblike/shiftcut` directly** (bypassing the ShiftCut CLI), set the env var yourself:

    ```bash
    GIT_CLONE_PROTECTION_ACTIVE=0 npx skills add Verblike/shiftcut
    ```

    This is tracked in [GH #316](https://github.com/Verblike/shiftcut/issues/316). An upstream fix in the `skills` CLI itself is the right long-term answer; until that lands, the env var is the correct workaround.
  </Tab>
</Tabs>

## shiftcut auth

Sign in to Verblike and manage credentials. Credentials are stored in
`~/.verblike/credentials` (mode `0600`) and are **shared with the
`verblike` CLI** — sign in with one and the other picks up the session.

Resolution order (first match wins):

1. `VERBLIKE_API_KEY` environment variable
2. `SHIFTCUT_API_KEY` environment variable (shiftcut alias)
3. `~/.verblike/credentials`

### Subcommands

#### `auth login --api-key`

Save a Verblike API key. The key is verified against `GET /v3/users/me`
before the command reports success; a rejected key is not left on disk.

```bash
# Interactive hidden-input prompt
shiftcut auth login --api-key

# Pipe a key from stdin (CI-friendly)
echo "$VERBLIKE_API_KEY" | shiftcut auth login --api-key
```
...
```bash
shiftcut auth status
shiftcut auth status --json   # machine-readable
```
...
```bash
shiftcut auth logout
shiftcut auth logout --keep-api-key   # clear only an OAuth session
shiftcut auth logout --yes            # skip the confirmation prompt
```
...
|----------|-------------|
| `VERBLIKE_API_KEY` | Override the stored credential. |
...
Render a ShiftCut composition on Verblike's hosted cloud — no local Chrome, no local ffmpeg, no AWS to manage. Sign in once with `shiftcut auth login` and the same credential drives every `cloud` subcommand.

```bash
shiftcut auth login                          # one-time
shiftcut cloud render ./my-video             # zip + upload + poll + download
shiftcut cloud render ./my-video --no-wait   # submit and exit with the render_id
shiftcut cloud list                          # browse recent renders
```
...
End-to-end render: zips the project (excluding root `renders`/`snapshots`, `.git`, `node_modules`, `dist`, `.next`, `coverage`, dotfiles, and project `.shiftcutignore` rules), uploads it through the direct-to-S3 asset flow, submits `POST /v3/shiftcut/renders`, polls `GET /v3/shiftcut/renders/{id}` until the render completes or fails, and streams the resulting video to disk.

Use `shiftcut cloud render --dry-run` to inspect the compressed size and largest included files without authenticating, uploading, or starting a render. Project-specific `.shiftcutignore` rules use gitignore syntax and also apply to `shiftcut publish`; keep rules narrow because dynamically selected assets may not have obvious static references.

Render parameters mirror the local `shiftcut render` UX where they overlap:
...
shiftcut cloud render --quality high --fps 60

# Fire-and-forget with a webhook (no local polling).
shiftcut cloud render --callback-url https://example.com/hf-hook --no-wait

# Re-render an already-uploaded composition (skips zip + upload).
shiftcut cloud render --asset-id asst_abc123

# Render from a public URL (no upload).
shiftcut cloud render --url https://cdn.example.com/site.zip
```
...
```bash
shiftcut cloud render . --idempotency-key "$(uuidgen)"
```
...
```bash
shiftcut cloud list
shiftcut cloud list --limit 50 --json
shiftcut cloud list --all
```
...
```bash
shiftcut cloud get hfr_abc123
shiftcut cloud get hfr_abc123 --json
```
...
```bash
shiftcut cloud delete hfr_abc123
shiftcut cloud delete hfr_abc123 --no-confirm --json
```

### When to pick `cloud` vs `lambda` vs local render

- `shiftcut render` (local): fastest iteration loop. Use during composition authoring.
- `shiftcut lambda render`: bring-your-own-AWS distributed rendering. Use when you've already invested in AWS and want chunked parallelism on your own account.
- `shiftcut cloud render`: zero-infra option. Verblike runs the render; you pay per credit. Use when you don't want to manage Chrome/ffmpeg/AWS locally.

### Auth + base URL

`cloud` reuses the credential resolved by `shiftcut auth status`. Override the API base for staging tests with `VERBLIKE_API_URL` (default `https://shiftcut.verblike.com`).

## shiftcut lambda

Deploy ShiftCut distributed rendering to AWS Lambda and drive renders from your laptop or CI.

The `shiftcut lambda` command group wraps the `@shiftcut/aws-lambda` SDK plus AWS SAM so an end-to-end render is three commands:

```bash
shiftcut lambda deploy
shiftcut lambda render ./my-project --width 1920 --height 1080 --wait
shiftcut lambda destroy   # when you're done
```
...
Builds `packages/aws-lambda/dist/handler.zip` and SAM-deploys the stack at `examples/aws-lambda/template.yaml`. On success, writes `<cwd>/.shiftcut/lambda-stack-<stackName>.json` so the other subcommands don't need to re-derive the bucket / state-machine ARN.

```bash
shiftcut lambda deploy \
  --stack-name=shiftcut-prod \
  --region=us-east-1 \
  --concurrency=8 \
  --memory=10240
```
...
```bash
shiftcut lambda sites create ./my-project
# → siteId: abc1234deadbeef0  (stable across re-runs of the same tree)

shiftcut lambda render ./my-project --site-id=abc1234deadbeef0 --width 1920 --height 1080
```
...
```bash
shiftcut lambda render ./my-project \
...
The composition can be parameterised with `--variables` / `--variables-file`, mirroring the local `shiftcut render` flags. Variables flow into the Step Functions execution input and reach every chunk worker as `window.__hfVariables`. Mismatches against the composition's `data-composition-variables` declaration print as warnings; pass `--strict-variables` to fail the command instead.

```bash
shiftcut lambda render ./my-template --site-id=abc1234deadbeef0 \
...
```bash
shiftcut lambda render-batch ./my-template \
  --batch ./users.jsonl \
  --width 1920 --height 1080 \
  --max-concurrent 10
```
...
Pass `--json` for the machine-readable form. Poll each execution via `shiftcut lambda progress <renderId>` (or use the returned `executionArn`).
...
```bash
shiftcut lambda render-batch ./my-template --batch ./users.jsonl \
...
```bash
shiftcut lambda progress hf-render-abcd1234
```
...
shiftcut lambda policies user

# Print { TrustRelationship, InlinePolicy } for an IAM role (default: cloudformation principal).
shiftcut lambda policies role --principal=cloudformation

# Validate a checked-in policy still covers the CLI's needs.
shiftcut lambda policies validate ./infra/iam/shiftcut-deploy.json
```
...
`shiftcut lambda` keeps per-stack metadata under `<cwd>/.shiftcut/lambda-stack-<name>.json` so the verbs don't need to call `describe-stacks` every time. Commit the file to a repo or `.gitignore` it depending on your workflow — it contains the bucket name, state-machine ARN, and region, none of which are secrets but all of which are AWS-account-identifying.

## shiftcut cloudrun

The Google Cloud counterpart to `shiftcut lambda`. Deploys ShiftCut distributed rendering to Cloud Run + Cloud Workflows and drives renders from your laptop or CI. Wraps the `@shiftcut/gcp-cloud-run` SDK plus `terraform` (the module shipped with the package) and `gcloud` / Cloud Build for the image.

```bash
shiftcut cloudrun deploy --project my-gcp-project
shiftcut cloudrun render ./my-project --width 1920 --height 1080 --wait
shiftcut cloudrun destroy --project my-gcp-project   # when you're done
```
...
```bash
shiftcut cloudrun deploy --project my-gcp-project --region us-central1
shiftcut cloudrun deploy --project my-gcp-project --image us-central1-docker.pkg.dev/my-gcp-project/Verblike/shiftcut-render:v1
```

Flags: `--project` (required), `--region` (default `us-central1`), `--image` (skip the build), `--repo` (Artifact Registry repo, default `shiftcut`). Machine sizing / scaling: `--cpu` (1/2/4/8, default 4), `--memory` (e.g. `32Gi`, default `16Gi`), `--max-instances` (render fan-out ceiling, default 100), `--timeout` (per-request seconds, max 3600). Omitted sizing flags keep the module defaults; for anything finer, apply the Terraform module directly.
...
`shiftcut cloudrun` caches the deployed stack's coordinates under `~/.shiftcut/cloudrun-state.json` (project id, region, bucket, service URL, workflow id) so `render` / `progress` / `destroy` don't need them re-passed. None are secrets, but all are GCP-project-identifying.

## shiftcut.json

`shiftcut init` writes a `shiftcut.json` file at the root of every new project. `shiftcut add` reads it to know which registry to pull items from and where to drop them. Edit the file (or delete it to fall back to defaults) to reshape your project layout or point at a custom registry.
...
| Field | Description |
|-------|-------------|
| `registry` | Base URL of the registry `add` pulls from. Defaults to the public Shiftcut registry. |
...
<CardGroup cols={2}>
  ...
  <Card title="Studio" icon="palette" href="/packages/studio">
    The editor UI that powers `shiftcut preview`. Use directly to embed in your own app.
  </Card>
