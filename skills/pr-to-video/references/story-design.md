...
This file defines the story: what the video explains, in what order, and why each frame exists. It does not define layout, effects, animation, or file syntax. For exact storyboard syntax follow `../shiftcut-core/references/storyboard-format.md` and `../shiftcut-core/references/script-format.md`.

## Read first

1. `shiftcut.json` — locked brief: angle (archetype), audience, length, aspect, language.
...
**Value before evidence** (`../shiftcut-creative/references/story-spine.md`): the viewer-facing payoff — what the change unlocks, fixes, or speeds up — lands by the second beat; the diff and the mechanism are the **evidence** for that claim, never the opening. Implementation is the footnote of the story, not the spine.
