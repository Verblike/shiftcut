...
#             `npx skills add Vadagon/shiftcut` does for a real user)
#   • CLI    → wired via a `file:` dep so `npx shiftcut` resolves to the LOCAL
...
#
# Why a sandbox (and not `npx skills add Vadagon/shiftcut#test/skills-fresh`):
#   `skills add` only copies skills/. The capture tool you changed lives in
#   packages/cli (the @shiftcut/cli package), so an online skills-only install
...
#      package.json (`file:` CLI dep). It does NOT init a shiftcut project here
#      — the video workflows run `npx shiftcut init` inside their own subdirs.
...
#      ~/.npm/_npx cache, so `npx shiftcut` resolves to the LOCAL build from any
...
# WORKSPACE ROOT, not a shiftcut project: the video workflows run
# `npx shiftcut init` inside their own subdirs, so a project at the root would
# make a skill find a stray composition here. We only need a package.json with the
# `file:` CLI dep so `npx shiftcut` (and the skills' init/render calls from
...
cat > package.json <<JSON
{
  ...
  "type": "module",
  "dependencies": {
    "shiftcut": "file:$HF_CLI_PKG"
  }
}
JSON

ok "package.json points shiftcut → file:$HF_CLI_PKG"
...
[[ -x "node_modules/.bin/shiftcut" ]] || fail "node_modules/.bin/shiftcut missing after install."
ok "node_modules/.bin/shiftcut → local CLI"

# --------- step 4.5: sandbox-private npm global prefix (npx-leak guard) ---------
# The file: dep only covers `npx shiftcut` run somewhere UNDER $TEST_DIR — npx
...
[[ -x "$NPM_GLOBAL_PREFIX/bin/shiftcut" ]] || fail "$NPM_GLOBAL_PREFIX/bin/shiftcut missing after install."
ok "npx shiftcut now resolves to the local build from ANY directory (launch with the env below)"
...
WORKFLOWS=(product-launch-video faceless-explainer embedded-captions \
           ...
           remotion-to-shiftcut slideshow)
DOMAIN=(shiftcut-core shiftcut-creative shiftcut-animation shiftcut-cli media-use shiftcut-registry)
