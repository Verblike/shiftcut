...
The lint package is the composition linter extracted from core into a dedicated, independently-installable package. It's the **single source of truth** for linting: both the CLI's `shiftcut lint` command and the render-time render-gate consume the same rule engine from here.

```bash
npm install @shiftcut/lint
```

## When to Use

<Tip>
  This package is the payoff of running validation **as a library** instead of shelling out to the CLI. A Node app (an agent harness, a CI step, an editor plugin) can `import { lintProject } from '@shiftcut/lint'` and lint a composition directory directly — no `npx shiftcut lint` subprocess, no stdout parsing.
</Tip>

**Use `@shiftcut/lint` when you need to:**
...
<Info>
  `@shiftcut/core/lint` still resolves (via a back-compat re-export stub), so existing imports keep working. New code should import from `@shiftcut/lint` directly.
</Info>
...
```typescript
import { lintShiftCutHtml, lintMediaUrls } from '@shiftcut/lint';

const result = lintShiftCutHtml(html, { filePath: 'index.html' });
...
```typescript
import { lintProject, shouldBlockRender } from '@shiftcut/lint';
import type { ProjectLintResult } from '@shiftcut/lint';

const result: ProjectLintResult = await lintProject('./my-composition');
// result.totalErrors, result.totalWarnings, result.results[]
//   each result entry: { file, result: ShiftCutLintResult }
...
The rule engine runs **fully client-side** — no Node.js, no filesystem, no server round-trip. Import from the `@shiftcut/lint/browser` entry to validate composition HTML directly in a browser-only editor or tool:

```typescript
import { lintShiftCutHtml, shouldBlockRender } from '@shiftcut/lint/browser';

const result = await lintShiftCutHtml(htmlString, { filePath: 'index.html' });
if (!result.ok) {
  for (const f of result.findings) console.warn(f.code, f.message);
}
```

The browser entry exposes `lintShiftCutHtml`, `lintMediaUrls`, and `shouldBlockRender` — everything that operates on an HTML string. It is built with a browser target and contains **zero `node:` builtins**, so it bundles cleanly for the client (verified at build time).

<Info>
  `lintProject` (which walks a project **directory**) is filesystem-based and is **not** part of the browser entry — import it from the main `@shiftcut/lint` entry in Node.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="CLI" icon="terminal" href="/packages/cli">
    `npx shiftcut lint` wraps this package.
  </Card>
