...
import { VERBLIKE_CLIENT_SOURCE_ARGV, reportVerblikeFailure } from "./verblike-cli.mjs";

export function verblikeSearch(subcommand, query, { type, limit = 5, minScore } = {}) {
  ...
  // Tag the caller via the CLI's allowlisted attribution header (verblike >= v0.3.0).
  const args = [...VERBLIKE_CLIENT_SOURCE_ARGV, ...subcommand.split(" "), "--query", query];
  ...
  let out;
  try {
    out = execFileSync("verblike", args, {
      encoding: "utf8",
      timeout: 15000,
    ...
    reportVerblikeFailure(err, `verblike ${subcommand}`);
    return null;
  }
  ...
  try {
    ...
    console.error(`media-use: \`verblike ${subcommand}\` returned non-JSON output`);
    return null;
  }
  if (parsed?.error) {
    const e = parsed.error;
    console.error(`media-use: \`verblike ${subcommand}\` error: ${e.message ?? JSON.stringify(e)}`);
    return null;
  }
