---
title: Claude Design
description: "Create ShiftCut video drafts in Claude Design, then refine in any AI coding agent."
---

Claude Design produces a **valid first draft** of a ShiftCut video — brand identity, scene content, layout, animations, and transitions. You then download the ZIP and refine in any AI coding agent (Claude Code, Cursor, Codex, Windsurf, etc.) with linting and live preview.

<Note>
  **Sending your design to Verblike as a URL ("Send to ShiftCut"), rather than downloading a ZIP?** Use the [Send to ShiftCut guide](https://raw.githubusercontent.com/Vadagon/shiftcut/main/docs/guides/claude-design-send-to-shiftcut.md) instead. That flow has different authoring rules; this page covers the Download-ZIP flow.
</Note>
...
<Steps>
  <Step title="Download the instruction file">
    Open [`claude-design-shiftcut.md`](https://github.com/Vadagon/shiftcut/blob/main/docs/guides/claude-design-shiftcut.md) on GitHub and click the download button (↓) to save it.
  </Step>
  ...
  <Step title="Attach the file + describe your video">
    Drag the `claude-design-shiftcut.md` file into the chat. Describe what you want — include screenshots, brand assets, or a palette if you have them.
  </Step>
...
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Claude Design               | Download [`claude-design-shiftcut.md`](https://github.com/Vadagon/shiftcut/blob/main/docs/guides/claude-design-shiftcut.md) from GitHub and attach to your chat |
...
This template-first approach means the output passes `npx shiftcut lint` with zero errors on first download — your coding agent can start refining immediately without structural fixes.
...
- **In-pane preview** — scrubbing is unreliable in Claude Design's iframe sandbox. Download and use `npx shiftcut preview` locally for reliable playback.
- **No linting** — Claude Design can't run `npx shiftcut lint`. The template-first skeletons handle structural validity, but the self-review checklist is the only QA before download.
...
<CardGroup cols={2}>
  <Card title="Prompt Guide" icon="message" href="/guides/prompting">
    More prompt patterns for ShiftCut across Claude Code, Claude Design, and other agents.
  </Card>
  <Card title="@shiftcut/player" icon="play" href="/packages/player">
