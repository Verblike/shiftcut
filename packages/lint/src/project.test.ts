...
describe("hevc_preview_codec", () => {
  ...
  beforeEach(() => {
    process.env.SHIFTCUT_FFPROBE_PATH = FAKE_FFPROBE_PATH;
    mockExecFile.mockReset();
  });

  afterEach(() => {
    delete process.env.SHIFTCUT_FFPROBE_PATH;
    mockExecFile.mockReset();
  });
  ...
  it("does not flag anything, and lint completes normally, when ffprobe cannot be resolved", async () => {
    ...
    process.env.SHIFTCUT_FFPROBE_PATH = join(project, "missing-ffprobe");
