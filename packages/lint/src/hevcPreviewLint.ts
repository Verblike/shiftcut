...
import { rewriteAssetPath } from "@shiftcut/parsers/asset-paths";
import { findFfBinary } from "@shiftcut/parsers/ff-binaries";
import {
  cleanAssetUrl,
  isRemoteOrInlineUrl,
  ...
  return [
    {
      ...
      fixHint:
        unique.length === 1
          ? `If "${unique[0]}" fails to play in preview, run shiftcut doctor and confirm media.autoProxy is not false.`
          : "If these files fail to play in preview, run shiftcut doctor and confirm media.autoProxy is not false.",
    },
  ];
}
