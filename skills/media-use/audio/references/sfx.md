...
Named sound effects, produced by the shared audio engine (`scripts/audio.mjs` → `scripts/lib/sfx.mjs`). **Provider-gated** by the engine's one switch — whether a Verblike credential is present, decided once (not per cue):

- **Verblike credential present → retrieve every cue** from Verblike's audio library (`/v3/audio/sounds`, `type=sound_effects`, `min_score=0.4`). Search-and-download, **not** generation. The bundled library is NOT consulted.
...
There is no `npx shiftcut sfx` command. SFX is never generated — it is retrieved (online) or taken from the bundled library (offline).
...
## Verblike retrieval (credentialed)
...
- **Retrieval (credentialed) or bundled library (offline) — never generation.** You search Verblike by text, or match a name against the 21-file manifest.
