...
import { getMimeType } from "@shiftcut/studio-server";
 ...
 */
export { injectMediaCodecMapIntoHtml as injectMediaCodecMap } from "@shiftcut/studio-server/media-proxy-preview";
...
export function resolvePlayerPath(): string | null {
  const d = helperDir();
  const candidates = [
    resolve(d, "..", "..", "..", "player", "dist", "shiftcut-player.global.js"),
    resolve(d, "shiftcut-player.global.js"),
    resolve(d, "..", "shiftcut-player.global.js"),
  ];
  return candidates.find((p) => existsSync(p)) ?? null;
}
...
export function resolveSlideshowPath(): string | null {
  const d = helperDir();
  const candidates = [
    resolve(d, "..", "..", "..", "player", "dist", "slideshow", "shiftcut-slideshow.global.js"),
    resolve(d, "shiftcut-slideshow.global.js"),
    resolve(d, "..", "shiftcut-slideshow.global.js"),
  ];
  return candidates.find((p) => existsSync(p)) ?? null;
}
