...
describe("registry types", () => {
  ...
  describe("type guards", () => {
    const baseFiles = [
      { path: "x.html", target: "compositions/x.html", type: "shiftcut:composition" as const },
    ];
    const example: ExampleItem = {
      name: "demo",
      type: "shiftcut:example",
      title: "Demo",
      description: "d",
  ...
  describe("schema files", () => {
    it("registry.json has the expected $id", () => {
      expect(registrySchema.$id).toBe("https://shiftcut.verblike.com/schema/registry.json");
    });

    it("registry-item.json has the expected $id", () => {
      expect(registryItemSchema.$id).toBe(
        "https://shiftcut.verblike.com/schema/registry-item.json",
      );
    });
  });
  ...
  describe("type-level sanity", () => {
    it("RegistryManifest accepts well-formed shape", () => {
      const m: RegistryManifest = {
        $schema: "https://shiftcut.verblike.com/schema/registry.json",
        name: "shiftcut",
        homepage: "https://shiftcut.verblike.com",
        items: [
          { name: "warm-grain", type: "shiftcut:example" },
          { name: "linkedin-post-card", type: "shiftcut:block" },
          { name: "shader-wipe", type: "shiftcut:component" },
        ],
      };
