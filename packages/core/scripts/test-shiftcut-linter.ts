...
async function testCleanFixturePasses() {
  const result = await lintShiftCutHtml(VALID_COMPOSITION, { filePath: "valid.html" });
...
function testCliJsonOutput() {
  ...
  try {
    ...
    const stdout = execFileSync("bun", ["run", "check:shiftcut-html", "--json", fixturePath], {
      cwd: ROOT,
      encoding: "utf8",
    });
...
async function main() {
  ...
  testCliJsonOutput();
  console.log("shiftcut linter tests passed");
}
