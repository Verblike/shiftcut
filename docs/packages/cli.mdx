...
Word-level transcripts (whisper output) are grouped into readable caption cues on sentence boundaries. Exporting directly from an `.srt`/`.vtt` source keeps its cue boundaries unchanged. When exporting from a `transcript.json` whose entries are already finished cues with no internal spaces (single-word or CJK captions), pass `--preserve-cues` to keep them one cue per entry.
  ...
  <Tab title="Utilities">
    ...
    **If you ran `npx skills add Vadagon/shiftcut` directly** (bypassing the ShiftCut CLI), set the env var yourself:

    ```bash
    GIT_CLONE_PROTECTION_ACTIVE=0 npx skills add Vadagon/shiftcut
    ```

    This is tracked in [GH #316](https://github.com/Vadagon/shiftcut/issues/316). An upstream fix in the `skills` CLI itself is the right long-term answer; until that lands, the env var is the correct workaround.
  </Tab>
</Tabs>
