    ...
    for (const { pattern, kind, targetIndex } of directIdPatterns) {
      ...
      while ((match = pattern.exec(script.content)) !== null) {
        ...
        findings.push({
          ...
          severity: "error",
          message: `Inline <script> imperatively controls managed media via ${kind}. ShiftCut must own media play/pause/seek to keep preview, timeline, and renders deterministic.`,
    ...
    for (const tag of tags) {
      ...
      if (hasSrc && !hasDataStart) {
        findings.push({
          ...
          severity: "error",
          message: `<${tag.name}${hasId ? ` id="${hasId}"` : ""}> has src but no data-start. ShiftCut cannot own playback for untimed media, so preview and render behavior can diverge.`,
