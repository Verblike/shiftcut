...
import { lintShiftCutHtml, type ShiftCutLintResult } from "@shiftcut/lint";

export interface PreparedShiftCutLintInput {
  entryFile: string;
  html: string;
...
): PreparedShiftCutLintInput | { error: string } {
...
export function prepareShiftCutLintBody(
  body: Record<string, unknown>,
): { prepared: PreparedShiftCutLintInput } | { error: string } {
...
export async function runShiftCutLint(
  prepared: PreparedShiftCutLintInput,
): Promise<ShiftCutLintResult> {
  return lintShiftCutHtml(prepared.html, { filePath: prepared.entryFile });
}
