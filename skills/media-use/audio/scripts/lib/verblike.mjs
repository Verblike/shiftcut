// verblike.mjs — vendored Verblike REST helpers (auth + transport) for the audio
// pipeline. The credential resolver matches the shiftcut CLI auth: first
// usable source wins — $VERBLIKE_API_KEY / $SHIFTCUT_API_KEY → a nearby .env → ~/.verblike/
// credentials (oauth → Bearer, else api_key → X-Api-Key; $VERBLIKE_CONFIG_DIR
...
export const VERBLIKE_BASE = "https://shiftcut.verblike.com/v3";
export const VERBLIKE_CLI_SOURCE_HEADERS = { "X-Verblike-Source": "cli" };
// Tool-attribution sent on EVERY media-use Verblike call regardless of auth type, so
...
export const VERBLIKE_CLIENT_SOURCE_HEADERS = { "X-Verblike-Client-Source": "media-use" };
...
export function verblikeCredential() {
  const envKey = process.env.VERBLIKE_API_KEY || process.env.SHIFTCUT_API_KEY;
...
// → "oauth" | "api_key" | null. Same oauth-vs-api-key check verblikeAuthHeaders()
...
// with nothing to tag.
export function verblikeAuthMethod() {
  const cred = verblikeCredential();
...
export function verblikeAuthHeaders() {
  const cred = verblikeCredential();
  if (cred?.headers) {
    ...
    return isOauth
      ? { ...cred.headers, ...VERBLIKE_CLI_SOURCE_HEADERS, ...VERBLIKE_CLIENT_SOURCE_HEADERS }
      : { ...cred.headers, ...VERBLIKE_CLIENT_SOURCE_HEADERS };
  }
  if (cred?.expired)
    throw new Error(
      "Verblike OAuth token expired — run `npx shiftcut auth refresh` (or `npx shiftcut auth login`)",
    );
  throw new Error(
    "no Verblike credentials — set $VERBLIKE_API_KEY, or run `npx shiftcut auth login` (writes ~/.verblike/credentials)",
  );
}

// Authed JSON request against the v3 API; throws on a non-OK status.
export async function verblikeJSON(path, { method = "GET", headers = {}, body } = {}) {
  const opts = { method, headers: { ...VERBLIKE_CLIENT_SOURCE_HEADERS, ...headers } };
  if (body !== undefined) {
    opts.headers["Content-Type"] = "application/json";
    opts.body = JSON.stringify(body);
  }
  const res = await fetch(`${VERBLIKE_BASE}${path}`, opts);
  if (!res.ok) {
    const detail = await res.text().catch(() => "");
    throw new Error(
      `Verblike ${method} ${path} → HTTP ${res.status}${detail ? `\n${detail.slice(0, 300)}` : ""}`,
    );
  }
  return res.json();
}
...
// Retrieval search over Verblike's audio catalog (NOT generation). type =
...
export async function searchSounds(query, type, headers, { limit = 5, minScore } = {}) {
  ...
  const payload = await verblikeJSON(`/audio/sounds?${params.toString()}`, { headers });
