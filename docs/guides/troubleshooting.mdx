...
description: "Solutions for common Shiftcut issues."
---
...
<AccordionGroup>
  <Accordion title='"No composition found"'>
    ...
    **Fix:** Run `npx shiftcut init` to create a composition from an [example](/examples), or verify your `index.html` has the correct structure:
  ...
  <Accordion title='"FFmpeg not found"'>
    ...
    After installing, run `npx shiftcut doctor` to verify the CLI can find it.

    <Tip>
      If you cannot install FFmpeg, use [Docker mode](/guides/rendering) instead — it bundles FFmpeg inside the container: `npx shiftcut render --docker --output output.mp4`
    </Tip>
  </Accordion>

  <Accordion title="Lint errors">
    Run `npx shiftcut lint` to check for common structural issues (see [CLI: lint](/packages/cli#lint)):
  ...
  <Accordion title="Preview not updating">
    ...
    2. Stop and restart `npx shiftcut preview`
  ...
  <Accordion title="Preview stutters or plays at a low frame rate">
    ...
    **Temporary workaround:** render to mp4 and watch the output. Render is accurate regardless of per-frame cost.
    ...
    ```bash Terminal
    npx shiftcut render --quality draft --output preview.mp4
    ```
  ...
  <Accordion title="Docker mode fails to start">
    ...
    - **Image pull fails:** Check your internet connection; the first render downloads the Shiftcut Docker image
  </Accordion>
  ...
  <Accordion title="Render is slow">
    ...
    2. Run `npx shiftcut benchmark` to find the optimal worker count for your system
  ...
  <Accordion title="Video asset plays black in preview but renders fine (HEVC/H.265)">
    ...
    1. Confirm auto-proxying is not disabled: no `--no-proxy` flag, and `media.autoProxy` is not `false` in `shiftcut.json`.
    2. Run `npx shiftcut doctor` to confirm ffmpeg/ffprobe are installed; the proxy transcode needs both.
    3. Run `npx shiftcut lint --verbose`: the info-level `hevc_preview_codec` finding names every affected asset.
...
Run `npx shiftcut doctor` to check your environment:
...
1. Run `npx shiftcut info` to gather system and project details
2. Check [GitHub Issues](https://github.com/Vadagon/shiftcut/issues) for similar reports
3. Open a new issue with the output of `npx shiftcut info` and steps to reproduce
